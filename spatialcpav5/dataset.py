...
the shared :class:`~spatialcpav5.data.SliceStack` global table and builds:
...
class TripletTokenDataset(Dataset):
    ...
    samples
        ...
        :func:`~spatialcpav5.data.build_triplet_samples`.
    n_neighbors
...
def _duplicate_pad_mask(tok_idx: np.ndarray, k: int) -> np.ndarray:
    ...
    which case :func:`~spatialcpav5.data.knn_indices` repeats the last neighbor.
