"""
Configuration objects for SpatialCPA-v5 (transformer version).
...
The top-level :class:`SpatialCPAv5Config` bundles the sub-configs and provides
...
class ModelConfig:
    ...
    expression_encoder
        ...
        registry in :mod:`spatialcpav5.encoders` makes it trivial to add
...
class TrainConfig:
    ...
    checkpoint_dir: str = "checkpoints/spatialcpav5"
...
class InferenceConfig:
    ...
    # v5 default is "transfer": it preserves cell-to-cell variance and the real
    # gene-gene structure, which is what the correspondence-free evaluation scores.
    ...
    #                z-interpolated position. The cell COUNT is the z-interpolated
    #                flanking count and the density/morphology track the true tissue
    #                (adjacent sections change slowly), so placement and count are
    #                far more accurate than a density-field integral or a uniform
    #                grid. Uses only the (training) flanking slices — leakage-safe.
    #   "density"  — fully de-novo: sample positions ∝ the predicted density field
    #                and set the count from its integral.
    #   "flanking" — the real (x, y) of BOTH flanking slices stacked (doubles the
    #                density; kept for ablation).
    #   "grid"     — a uniform lattice over the bounding box (uniform density).
    position_source: str = "interpolate"

    # Cell-type annotation source at generation:
    #   "transfer" — (v5 default) copy the cell type of the same nearest training
    #                cell that expression is transferred from, so the synthesized
    #                cell's type, expression and neighborhood are mutually
    #                consistent. This makes the cell-type composition and spatial
    #                organization track the flanking (training) slices, which for
    #                adjacent sections closely match the held-out slice.
...
# --------------------------------------------------------------------------- #
@dataclass
class SpatialCPAv5Config:
    """Complete configuration for a SpatialCPA-v5 experiment."""
    ...
    @classmethod
    def from_dict(cls, d: Dict[str, Any]) -> "SpatialCPAv5Config":
    ...
    @classmethod
    def load(cls, path: str | Path) -> "SpatialCPAv5Config":
        with open(path) as f:
