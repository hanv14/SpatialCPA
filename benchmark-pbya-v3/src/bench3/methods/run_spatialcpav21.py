"""SpatialCPA-v21 (H3D-FLA + coherent mix + field-aligned grounding) — v3 wrapper.

Like the v16/v18/v19/v20 wrappers, this lives in ``benchmark-pbya-v3`` rather than
beside the others in ``benchmark-pbya-v2/src/benchmark/methods/``, because v2 is
frozen and never ran v21. It speaks the identical ``_v2_io`` contract as every
...
v21 is the single-file re-implementation ``learn_spatialcpav21.py`` (v20 +
coherent mix + field-aligned grounding). It keeps v20's Bernoulli cross-mix but
draws every per-gene mixing mask from smooth random fields — so neighbours flip
the same genes together and the spatial autocorrelation ranking survives the mix
— and adds gap-adaptive re-grounding/layout plus field-aligned re-grounding and
per-gene field repair, every one of them still emitting only real measurements.
It is a *module file at the repository root*, not an installed package, so — like
v18/v19/v20 — this wrapper loads the file directly. The public API is v14's
(``Slice`` / ``SliceStack`` / ``SpatialCPAv14`` / ``V14Config`` / ``VirtualSlice``);
``Slice`` carries an optional ``raw_expression`` and ``V14Config`` gains the
v18/v19/v20/v21 knobs.
...
# (Same wiring as the v16/v18/v19/v20 wrappers: this file sits four parents below
# the repo root, so parents[4] is SpatialCPA/ and benchmark-pbya-v2 is beside v3.)
...
# The v21 method is a single file, not a package: name the FILE, or the directory
...
FILE_ENV = "SPATIALCPAV21_FILE"      # full path to learn_spatialcpav21.py
ROOT_ENV = "SPATIALCPAV21_ROOT"      # directory that CONTAINS the file
_MODULE_NAME = "learn_spatialcpav21"
_FILENAME = "learn_spatialcpav21.py"


def _candidate_files():
    """Paths that might be ``learn_spatialcpav21.py``, most-specific first."""
...
def _load_module():
    """Import ``learn_spatialcpav21.py`` from disk under a stable module name.
...
_V21, _V21_PATH, _TRIED = _load_module()
...
def check_environment():
    ...
    try:
        ...
        # v21 degrades to the numpy latent-grounded fallback without torch — a
    ...
    print(f"spatialcpav21 (learn_spatialcpav21.py) from {_V21_PATH}; {t}")
    return True
...
def _normalize_expression(adata):
    ...
    Identical policy to the v14/v16/v18/v19/v20 wrappers, so every method receives
...
def build_stack(adata, X_log, X_raw, ct_all):
    ...
    ``learn_spatialcpav21`` ``main()``, which does the same with ``X_raw=vol.X``).
    """
    Slice, SliceStack = _V21.Slice, _V21.SliceStack
...
def _build_config(args):
    """A ``V14Config`` whose defaults reproduce v21; flags only override knobs.

    The argparse defaults below mirror ``V14Config``'s own defaults, so running
    the wrapper with just the shared generation-only arguments runs v21 as
    intended and cannot silently drift from this file. The many fine-grained v21
    sub-knobs (repair_q/noise/frac, field_align_k/cand_k/frac/margin, ...) are not
    surfaced as flags; leaving them untouched keeps them at their V14Config
    defaults, which is exactly the production v21 configuration.
    """
    cfg = _V21.V14Config()
    cfg.seed = args.seed
    cfg.verbose = True
    ...
    # ── v19 gap-adaptive weights (alpha; drives cross-mix + the v21 ramps) ──
    ...
    # ── v21 additions (coherent mix + field-aligned grounding) ──
...
def run_method(adata, targets, gene_names, X_log, X_raw, args):
    ...
    # exactly as the v14/v16/v18/v19/v20 wrappers do.
    ...
    print(f"  epochs(A={cfg.pretrain_epochs},B={cfg.epochs}), "
          ...
          f"cross_mix={cfg.cross_mix}, coherent_mix={cfg.coherent_mix}"
          f"(freq={cfg.mix_field_freq},cand_k={cfg.mix_cand_k}), "
          f"ground_gap_adapt={cfg.ground_gap_adapt}"
          f"(narrow={cfg.ground_margin_narrow}), "
          f"layout_gap_adapt={cfg.layout_gap_adapt}(narrow={cfg.layout_freq_narrow}), "
          f"field_align={cfg.field_align}(gap_only={cfg.field_align_gap_only}), "
          f"field_repair={cfg.field_repair}(tail={cfg.repair_tail_scale}), "
    ...
    # Matched by config.invalid_log_markers: a False here means v21 fell back to
...
def main():
    p = argparse.ArgumentParser(
        description="SpatialCPA-v21 wrapper (H3D-FLA + coherent mix + field-aligned grounding)")
    ...
    # ── v19 gap-adaptive weights (still active: alpha drives cross-mix + v21 ramps) ──
    p.add_argument("--no-gap-adapt", action="store_true",
                   help="ablate gap adaptation: hold alpha=0 so v21 reduces to v18 "
                        "at every gap width")
    ...
    # ── v21-specific knobs (coherent mix + field-aligned grounding) ──
    ...
    p.add_argument("--gene-mix-wide", type=float, default=0.7,
                   help="gap-adaptive gene-mix rate: the per-gene redraw fraction "
                        "ramps from --gene-mix-frac (alpha=0) to this (alpha=1); "
                        "<=0 disables the ramp")
    ...
    p.add_argument("--no-layout-gap-adapt", action="store_true",
                   help="ablate gap-adaptive layout granularity: use a fixed patch "
                        "frequency at every gap width")
    p.add_argument("--layout-freq-narrow", type=float, default=8.0,
                   help="coherent-patch frequency at alpha=0 (narrow gaps); ramps "
                        "down to --coherent-source's frequency at alpha=1")
    ...
    print(f"Running SpatialCPA-v21 for targets "
