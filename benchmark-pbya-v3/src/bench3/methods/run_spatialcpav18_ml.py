...
across the five learners on a fixture whose real zero fraction was 0.39. So a
...
def check_environment(learner):
    ...
    try:
        ...
        print(f"ERROR: scikit-learn is required by run_spatialcpav18_ml "
...
def check_output_size(adata, n_targets, max_gb):
    ...
    front rather than discovered after the flow has trained.
    """
...
# ── learners (identical to the v14 / v21 families, so all three compare) ─────
...
def build_parser():
    ...
    p.add_argument("--learner", required=True, choices=list(LEARNERS),
                   help="expression regressor replacing v18's donor-copy step")
    ...
    p.add_argument("--max-dense-gb", type=float, default=8.0,
                   help="refuse before training if the dense Q x G prediction would "
                        "exceed this (a regressor emits no zeros)")
