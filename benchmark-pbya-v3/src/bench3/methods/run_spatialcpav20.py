"""SpatialCPA-v20 (H3D-FLA + Bernoulli cross-mix) — benchmark-pbya-v3 wrapper.

Like the v16/v18/v19 wrappers, this lives in ``benchmark-pbya-v3`` rather than
beside the others in ``benchmark-pbya-v2/src/benchmark/methods/``, because v2 is
frozen and never ran v20. It speaks the identical ``_v2_io`` contract as every
...
v20 is the single-file re-implementation ``learn_spatialcpav20.py`` (v18 +
Bernoulli cross-mix, repairing v19's wide-gap collapse). Where v19 convex-
interpolated two sparse count profiles — densifying every gene nonzero in either
parent — v20 does a per-gene Bernoulli selection between the exemplar and a
cross-flank same-type partner: the expectation is the same linear interpolation,
but every emitted value stays a real measurement, so sparsity, dispersion and
count-ness hold by construction. An alpha tolerance snaps narrow-gap designs to
exactly v18. It is a *module file at the repository root*, not an installed
package, so — like v18/v19 — this wrapper loads the file directly. The public API
is v14's (``Slice`` / ``SliceStack`` / ``SpatialCPAv14`` / ``V14Config`` /
``VirtualSlice``); ``Slice`` carries an optional ``raw_expression`` and
``V14Config`` gains the v18/v19/v20 knobs.
...
# (Same wiring as the v16/v18/v19 wrappers: this file sits four parents below the
...
# The v20 method is a single file, not a package: name the FILE, or the directory
...
FILE_ENV = "SPATIALCPAV20_FILE"      # full path to learn_spatialcpav20.py
ROOT_ENV = "SPATIALCPAV20_ROOT"      # directory that CONTAINS the file
_MODULE_NAME = "learn_spatialcpav20"
_FILENAME = "learn_spatialcpav20.py"


def _candidate_files():
    """Paths that might be ``learn_spatialcpav20.py``, most-specific first."""
...
def _load_module():
    """Import ``learn_spatialcpav20.py`` from disk under a stable module name.
...
_V20, _V20_PATH, _TRIED = _load_module()
...
def check_environment():
    ...
    try:
        ...
        # v20 degrades to the numpy latent-grounded fallback without torch — a
    ...
    print(f"spatialcpav20 (learn_spatialcpav20.py) from {_V20_PATH}; {t}")
    return True
...
def _normalize_expression(adata):
    ...
    Identical policy to the v14/v16/v18/v19 wrappers, so every method receives the
...
def build_stack(adata, X_log, X_raw, ct_all):
    ...
    the pre-normalization measurement carried alongside so raw_output / the v20
    Bernoulli cross-mix can emit real profiles verbatim (see ``learn_spatialcpav20``
    ``main()``, which does the same with ``X_raw=vol.X``).
    """
    Slice, SliceStack = _V20.Slice, _V20.SliceStack
...
def _build_config(args):
    """A ``V14Config`` whose defaults reproduce v20; flags only override knobs.

    The argparse defaults below mirror ``V14Config``'s own defaults, so running
    the wrapper with just the shared generation-only arguments runs v20 as
    ...
    # ── v19 gap-adaptive weights (alpha; still drives the v20 cross-mix) ──
    cfg.gap_adapt = not args.no_gap_adapt
    cfg.gap_scale = args.gap_scale
    cfg.pair_interp_max = args.pair_interp_max   # retired in v20; kept for compat
    # ── v20 additions (Bernoulli cross-mix) ──
    cfg.cross_mix = not args.no_cross_mix
    cfg.alpha_tol = args.alpha_tol
    # v20 defaults these OFF (decode blur / inert curriculum hurt v19's mixing);
    # the argparse defaults preserve that, and the flags enable them for ablation.
...
def run_method(adata, targets, gene_names, X_log, X_raw, args):
    SpatialCPAv14 = _V20.SpatialCPAv14
    ...
    # exactly as the v14/v16/v18/v19 wrappers do.
    ...
    print(f"  epochs(A={cfg.pretrain_epochs},B={cfg.epochs}), "
          ...
          f"(scale={cfg.gap_scale}), cross_mix={cfg.cross_mix}"
          f"(alpha_tol={cfg.alpha_tol}), edit_gap_extra={cfg.edit_gap_extra}, "
          f"curriculum={cfg.curriculum_flow}, edit_w={cfg.edit_weight}, "
    ...
    # Matched by config.invalid_log_markers: a False here means v20 fell back to
...
def main():
    p = argparse.ArgumentParser(
        description="SpatialCPA-v20 wrapper (H3D-FLA + Bernoulli cross-mix)")
    ...
    # ── v19 gap-adaptive weights (still active: alpha drives the v20 cross-mix) ──
    p.add_argument("--no-gap-adapt", action="store_true",
                   help="ablate gap adaptation: hold alpha=0 so v20 reduces to v18 "
                        "at every gap width")
    ...
    p.add_argument("--pair-interp-max", type=float, default=1.0,
                   help="RETIRED in v20 (kept for API compat; convex interpolation "
                        "densified sparse counts and is replaced by --cross-mix). "
                        "No effect while --cross-mix is on.")
    # ── v20-specific knobs (Bernoulli cross-mix) ──
    p.add_argument("--no-cross-mix", action="store_true",
                   help="ablate the v20 Bernoulli cross-mix (per-gene real-value "
                        "selection between exemplar and cross-flank partner); with "
                        "it off, v20 reproduces v18 exactly")
    p.add_argument("--alpha-tol", type=float, default=1e-3,
                   help="alpha below this snaps to 0 so narrow-gap designs are "
                        "exactly v18 (guards the binary raw-output gate)")
    p.add_argument("--edit-gap-extra", type=float, default=0.0,
                   help="extra flow-decode blend at alpha=1 (added to --edit-weight, "
                        "capped 0.6). Default 0 in v20 (the decode blur hurt mixing); "
                        "raise only for ablation")
    p.add_argument("--curriculum-flow", action="store_true",
                   help="enable the wide-gap training curriculum (OFF by default in "
                        "v20 — inert at edit_weight=0, only costs training time)")
    ...
    # Capture the RAW measurement before normalization (raw_output / cross-mix
    # emit it verbatim), then log-normalize the model input in place.
    X_raw = _to_dense_f32(adata.X)
    et = _normalize_expression(adata)
    X_log = _to_dense_f32(adata.X)
    print(f"  expression_type={et}")

    print(f"Running SpatialCPA-v20 for targets "
