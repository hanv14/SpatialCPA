"""SpatialCPA-v23 (module chimerism / stratified mix / local composition field /
paired position morph) — benchmark-pbya-v3 wrapper.

Like the v16/v18/v19/v20/v21/v22 wrappers, this lives in ``benchmark-pbya-v3``
rather than beside the others in ``benchmark-pbya-v2/src/benchmark/methods/``,
because v2 is frozen and never ran v23. It speaks the identical ``_v2_io``
contract as every other wrapper — same CLI, same ``prediction.h5`` format, same
leakage guard — so ``run_benchmark`` invokes it exactly as it invokes the rest and
the evaluator reads its output unchanged.

v23 is the single-file re-implementation ``learn_spatialcpav23.py`` (built on v20,
not v21/v22). Its contribution is the "principled middle" between SpatialZ's
per-gene chimerism (maximal embedding novelty, but gene-gene covariance destroyed)
and v20's whole-profile copies (covariance intact, but low novelty): chimerism at
the granularity of CO-EXPRESSION MODULES — the coarsest unit that still creates
combinatorial novelty and the finest that cannot break within-module covariance.
Supporting mechanisms: niche-weighted partner selection, a variance-reduced
(stratified) cross-mix, local composition-field matching, and paired position
morphing. Every emitted expression value is still a real measurement. It is a
*module file at the repository root*, not an installed package, so — like v18-v22 —
this wrapper loads the file directly. The public API is v14's (``Slice`` /
``SliceStack`` / ``SpatialCPAv14`` / ``V14Config`` / ``VirtualSlice``); ``Slice``
carries an optional ``raw_expression`` and ``V14Config`` gains the v18/v19/v20/v23
knobs.
...
# (Same wiring as the v16/v18/v19/v20/v21/v22 wrappers: this file sits four
# parents below the repo root, so parents[4] is SpatialCPA/ and benchmark-pbya-v2
# is beside v3.)
...
# The v23 method is a single file, not a package: name the FILE, or the directory
...
FILE_ENV = "SPATIALCPAV23_FILE"      # full path to learn_spatialcpav23.py
ROOT_ENV = "SPATIALCPAV23_ROOT"      # directory that CONTAINS the file
_MODULE_NAME = "learn_spatialcpav23"
_FILENAME = "learn_spatialcpav23.py"


def _candidate_files():
    """Paths that might be ``learn_spatialcpav23.py``, most-specific first."""
...
def _load_module():
    """Import ``learn_spatialcpav23.py`` from disk under a stable module name.
...
_V23, _V23_PATH, _TRIED = _load_module()
...
def check_environment():
    ...
    try:
        ...
        # v23 degrades to the numpy latent-grounded fallback without torch — a
    ...
    print(f"spatialcpav23 (learn_spatialcpav23.py) from {_V23_PATH}; {t}")
    return True
...
def _normalize_expression(adata):
    ...
    Identical policy to the v14/v16/v18/v19/v20/v21/v22 wrappers, so every method
...
def build_stack(adata, X_log, X_raw, ct_all):
    ...
    the pre-normalization measurement carried alongside so raw_output / the module
    chimerism and cross-mix can emit real profiles verbatim (see
    ``learn_spatialcpav23`` ``main()``, which does the same with ``X_raw=vol.X``).
    """
    Slice, SliceStack = _V23.Slice, _V23.SliceStack
...
def _build_config(args):
    """A ``V14Config`` whose defaults reproduce v23; flags only override knobs.

    The argparse defaults below mirror ``V14Config``'s own defaults, so running
    the wrapper with just the shared generation-only arguments runs v23 as
    ...
    # ── v19 gap-adaptive weights (alpha; drives cross-mix + the v23 ramps) ──
    ...
    # ── v23 additions (module chimerism + niche/stratified/local-comp/morph) ──
    cfg.module_mix = not args.no_module_mix
    cfg.n_gene_modules = args.n_gene_modules
    cfg.niche_weight = args.niche_weight
    cfg.cross_mix_stratified = not args.no_cross_mix_stratified
    cfg.composition_local = not args.no_composition_local
    cfg.comp_bins = args.comp_bins
    cfg.comp_min_cells = args.comp_min_cells
    cfg.pair_morph = not args.no_pair_morph
    cfg.pair_morph_strength = args.pair_morph_strength
    cfg.pair_morph_cap = args.pair_morph_cap
    return cfg


def run_method(adata, targets, gene_names, X_log, X_raw, args):
    SpatialCPAv14 = _V23.SpatialCPAv14
    ...
    # exactly as the v14/v16/v18/v19/v20/v21/v22 wrappers do.
    ...
    print(f"  epochs(A={cfg.pretrain_epochs},B={cfg.epochs}), "
          ...
          f"raw_output={cfg.raw_output}, gap_adapt={cfg.gap_adapt}(scale={cfg.gap_scale}), "
          f"cross_mix={cfg.cross_mix}, module_mix={cfg.module_mix}"
          f"(n_modules={cfg.n_gene_modules}), niche_weight={cfg.niche_weight}, "
          f"stratified={cfg.cross_mix_stratified}, composition_local={cfg.composition_local}"
          f"(bins={cfg.comp_bins}), pair_morph={cfg.pair_morph}"
          f"(strength={cfg.pair_morph_strength},cap={cfg.pair_morph_cap}), "
    ...
    # Matched by config.invalid_log_markers: a False here means v23 fell back to
...
def main():
    p = argparse.ArgumentParser(
        description="SpatialCPA-v23 wrapper (module chimerism / stratified mix / "
                    "local composition field / paired position morph)")
    ...
    p.add_argument("--gene-mix-frac", type=float, default=0.15,
                   help="fraction of each cell's genes (or modules, when "
                        "--module-mix) resampled from a local same-type real cell")
    ...
    # ── v19 gap-adaptive weights (still active: alpha drives cross-mix + morph) ──
    p.add_argument("--no-gap-adapt", action="store_true",
                   help="ablate gap adaptation: hold alpha=0 so v23 reduces to v18 "
                        "at every gap width")
    ...
    p.add_argument("--no-cross-mix", action="store_true",
                   help="ablate the Bernoulli cross-mix (per-gene/-module real-value "
    ...
    # ── v23-specific knobs (the contribution) ──
    p.add_argument("--no-module-mix", action="store_true",
                   help="ablate module-level chimerism: mix per gene (v20 behaviour) "
                        "instead of per co-expression module")
    p.add_argument("--n-gene-modules", type=int, default=8,
                   help="number of co-expression modules (KMeans on gene PCA loadings)")
    p.add_argument("--niche-weight", type=float, default=1.0,
                   help="weight of morphology-vector cosine similarity when sampling "
                        "mix/cross partners (0 = v20's uniform/latent-only choice)")
    p.add_argument("--no-cross-mix-stratified", action="store_true",
                   help="ablate the variance-reduced cross-mix: draw iid Bernoulli "
                        "(v20) instead of exactly round(w*n) per (module, flank group)")
    p.add_argument("--no-composition-local", action="store_true",
                   help="ablate local composition-field matching: match only the "
                        "global type histogram (v14-v20 behaviour)")
    p.add_argument("--comp-bins", type=int, default=6,
                   help="composition grid is comp_bins^2 over the occupied plane")
    p.add_argument("--comp-min-cells", type=int, default=25,
                   help="bins with fewer cells fall back to the global composition pass")
    p.add_argument("--no-pair-morph", action="store_true",
                   help="ablate paired position morphing (move each cell along the "
                        "exemplar -> cross-flank-partner vector); OFF at narrow gaps anyway")
    p.add_argument("--pair-morph-strength", type=float, default=1.0,
                   help="0..1 scale on the morph vector")
    p.add_argument("--pair-morph-cap", type=float, default=3.0,
                   help="cap the morph displacement at this many median cell spacings")
    args = p.parse_args()
    ...
    # Capture the RAW measurement before normalization (raw_output / module mix /
    # cross-mix emit it verbatim), then log-normalize the model input in place.
    X_raw = _to_dense_f32(adata.X)
    et = _normalize_expression(adata)
    X_log = _to_dense_f32(adata.X)
    print(f"  expression_type={et}")

    print(f"Running SpatialCPA-v23 for targets "
    ...
    method_params = {
        ...
        # v23 module chimerism + supporting mechanisms
        "module_mix": not args.no_module_mix, "n_gene_modules": args.n_gene_modules,
        "niche_weight": args.niche_weight,
        "cross_mix_stratified": not args.no_cross_mix_stratified,
        "composition_local": not args.no_composition_local,
        "comp_bins": args.comp_bins, "comp_min_cells": args.comp_min_cells,
        "pair_morph": not args.no_pair_morph,
        "pair_morph_strength": args.pair_morph_strength,
        "pair_morph_cap": args.pair_morph_cap,
        "flow_matching": True, "generation_only": True,
    }
    _v2_io.write_prediction_h5(
        results, gene_names, target_sections, method_params, wall,
        args.output, "spatialcpav23_gen")
    return 0
