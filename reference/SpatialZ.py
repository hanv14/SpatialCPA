...
def Generate_spatialz(adata1, adata2, adata1_id='above', adata2_id='below',
                      ...
                      micro_env_key='mender', add_obs_list= None, verbose=True):
    """
    ...
    if syn_mode == 'default':
        ...
        start_time = time.time()
        adata3 = synthesize_gene_expression([adata1, adata2], adata3, cell_type_key, k_sam, micro_env_key)
...
def synthesize_gene_expression(reference_adatas, query_adata, cell_type_key, k_sam, micro_env_key):
...
def Generate_multiple_spatialz(adata1, adata2, num_sim, adata1_id='above', adata2_id='below',
                               ...
                               cell_type_key='cell_type',syn_mode= 'default', k_sam=3, micro_env_key = 'mender', add_obs_list=None, verbose=True,
                               include_raw=True):
    """
    ...
    for i in tqdm(range(1, num_sim), desc="Generating simulations"): 
        ...
        sim_adata = Generate_spatialz(adata1, adata2, adata1_id=adata1_id, adata2_id=adata2_id,
                                      ...
                                      cell_type_key=cell_type_key, syn_mode= syn_mode, k_sam=k_sam, micro_env_key = micro_env_key, add_obs_list=add_obs_list,verbose=True
                                      )
        # Create slice_id
...
def Generate_multiple_slices(adata_list, num_sim_list, adatas_id_list,
                             ...
                             num_projections=80, cell_type_key='cell_type', syn_mode= 'default', k_sam=3, micro_env_key='mender', add_obs_list=None,
    ...
    for i in tqdm(range(len(adata_list) - 1), desc="Generating simulations for slices"):
        ...
        for j in range(1, num_sim + 1):  # Varying alpha from 1 to 0 in num_sim steps
            ...
            sim_adata = Generate_spatialz(adata1, adata2, adata1_id=adata1_id, adata2_id=adata2_id,
                                          alpha=alpha, device=device, n_cell=n_cell, n_mag=n_mag, lr=lr,
                                          nb_iter_max=nb_iter_max, seed=seed, num_projections=num_projections,
                                          cell_type_key=cell_type_key, syn_mode= syn_mode, k_sam=k_sam, micro_env_key=micro_env_key, add_obs_list=add_obs_list, verbose=verbose
                                          )
