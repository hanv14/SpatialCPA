...
| R8 | ~~Reduced-epoch selection reverses gates with a training-free option~~ — **RESOLVED at T09**: `specs/09` §3 now carries the **training-free-option rule**, enforced by `TRAINING_FREE_OPTIONS` + an import-time check, merging `layout_mode` x `prior_mode` x `expr_mode` into one 18-cell full-budget gate. All six `cross-mix` cells rank bottom; **all three gates changed answer**, `layout_mode` included. New config `cbabb27aa44ee6e4`; the donor-bar score goes **2/6 → 4/6** |
| R9 | **New at T09**: `text_emb_mode` passes the rule but is decided at 600 steps under an unconverged `zinb-flow` incumbent (morans 0.60/0.65 vs 0.96 at full budget) and its winner flipped to `lookup`. Not R8's bias — both options are handicapped equally — but a gate decided on a model unlike the shipped one. **Open**, a spec question |
