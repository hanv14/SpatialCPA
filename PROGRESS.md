...
| R8 | ~~Reduced-epoch selection reverses gates with a training-free option~~ — **RESOLVED at T09**: `specs/09` §3 now carries the **training-free-option rule**, enforced by `TRAINING_FREE_OPTIONS` + an import-time check, merging `layout_mode` x `prior_mode` x `expr_mode` into one 18-cell full-budget gate. All six `cross-mix` cells rank bottom; **all three gates changed answer**, `layout_mode` included. New config `2ce15bbaf5cf2bc1`; the donor-bar score goes **2/6 → 4/6** |
...
| R10 | **New at T09**: same config, same seed, two processes → scores drift by up to **0.0120**, wider than the **0.0110** gap between `medcpt` and `lookup`. `text_emb_mode` is undecidable at one seed. Cause not established. Every T10 headline number inherits the envelope. **Open** |
