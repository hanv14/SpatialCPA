...
r"""
================================================================================
 learn_spatialcpav19.py   (v18 + GAP-ADAPTIVE GENERATION; see V19 CHANGES below)
...
class V14Config:
    ...
    pair_interp_max: float = 1.0          # at alpha=1, weight of the CROSS-FLANK
    ...
    edit_gap_extra: float = 0.30          # extra flow-decode blend at alpha=1
                                          # (added to edit_weight, capped 0.6)
    curriculum_flow: bool = True          # Phase B: with prob curriculum_p the
...
class SpatialCPAv14:
    ...
    def _generate(self, z):
        ...
        w_pair = alpha * cfg.pair_interp_max
        if w_pair > 0.0:
            partner2, ok2 = self._pair_across_flanks(anchor, pick, ct_idx, n_lo,
                                                     pool_nxy, pool_e, pool_type, rng)
            # exemplar profile sits at its own flank; partner at the other.
            # Interpolate at the cell's own flank-fraction so the mixture is
            # the molecular state AT z, per matched pair.
            t_lo = t                                   # weight of the UPPER flank
            in_lower = pick < n_lo
            w_other = np.where(in_lower, t_lo, 1.0 - t_lo)[:, None]
            interp = (1.0 - w_other) * pool_expr[pick] + w_other * pool_expr[partner2]
            interp[~ok2] = pool_expr[pick[~ok2]]
            expr = (1.0 - w_pair) * expr + w_pair * interp

        edit_eff = min(cfg.edit_weight + alpha * cfg.edit_gap_extra, 0.6)
        if edit_eff > 0.0 and cfg.edit_weight == 0.0 and alpha > 0.0:
            # gap-triggered flow decode blend even when the base config is copy-only
            dec = self.expr_latent.decode(e_hat_np * self._e_std + self._e_mean)
            expr = (1 - alpha * cfg.edit_gap_extra) * expr + alpha * cfg.edit_gap_extra * dec
        ...
        if raw_ok:
            ...
            expr = pool_raw[pick]
            if partner is not None:                      # apply the same mix on raw scale
