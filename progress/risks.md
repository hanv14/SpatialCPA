...
|---|---|---|---|---|
| R8 | **The 25 % reduced-budget heuristic is unsafe for `prior_mode`** (new at T09). `specs/09` §3 exempts the *budget* gate from the reduction because a reduced fit of a `2x` candidate is the `1x` candidate. The fixture shows the exemption is drawn too narrowly: at 600 steps `prior_mode=iid` outranks `correlated` (median rank 1.0 vs 2.0) and the selector applies that to a 2400-step model, where the joint gate's own `correlated` cells reach morans_pearson **0.9368** against the `iid` winner's **0.6494**. GATE 1 established that the correlated prior is the mechanism that makes the generated section's Moran's I track the tissue's at all — a property that needs training to express, which is exactly what a quarter budget removes. **Measured at both budgets** (`reports/r8_budget_grid.md`, 12 fits, 12 630 s) and the finding is wider than this row was written for: **both** of the selector's wrong choices are reduced-budget artefacts. `expr_mode` reverses too — `cross-mix` wins at 600 under both priors (rank 1.5) and comes **last** at 2400 under both (3.0). The mechanism is in one column: from 600 to 2400 steps `morans_pearson` gains **+0.3432** for `zinb-flow` and **−0.0180** for `cross-mix`, because the copying path needs no training and the flow path has barely started at a quarter budget. The two gates also **compound**, because coordinate descent is ordered: fixing `iid` first drops `zinb-flow` from rank 2.5 to 3.0 before the `expr_mode` gate is scored. At full budget the shipped config ranks **fifth of six**. **Fix is a design change** and is the spec owner's call; the proposal on the table is to merge `prior_mode` x `expr_mode` into one 6-cell gate scored at the selected budget, which fixes both reversals and the ordering effect for 6 full-budget fits. **The same pairing disables calibration outright:** with `prior_mode="correlated"` restored, `ell` calibration is still meaningless, because the selected `expr_mode="cross-mix"` takes every emitted count verbatim from a donor cell and never evaluates the flow — the donor weights come from the retrieval score, not from the latent, so the GRF is absent from the expression path entirely and the objective is constant to 10 decimal places across a 15× `ell_z` sweep. Whether `ell` can be calibrated at all is a property of the `prior_mode` × `expr_mode` pair, so the two gates cannot be chosen independently of the calibrator that runs after them; the calibrators now refuse both severing configurations rather than reporting a tie-break as a measurement | T09 | T10 | **open — recorded, not patched** |
