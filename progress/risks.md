...
| R10 | **Scores are reproducible only to ~0.012, which is wider than several gate margins** (new at T09). `medcpt` at 2400 steps fitted twice — same config, same seed, different process — moved by up to **0.0120** (`umap_mixing`), while the largest difference between `medcpt` and `lookup` is **0.0110**. Re-running the identical configuration moves the score as much as changing the gate does, so `text_emb_mode` is not resolved at one seed at any budget. Cause **not established**: both fits take explicit seeds, and nondeterministic float reduction under differing thread scheduling is the obvious suspect but was not confirmed. Not local to this gate — every headline number T10 reports inherits the same envelope. Proposals: a tie-break preferring the capability-preserving option below the envelope, and repeated seeds for any gate reaching a headline claim | T09 | T10 | **open — recorded, not patched** |
