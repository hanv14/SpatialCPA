"""Screen processed source datasets for fitness under the section protocol.

The benchmark runs one protocol: cut a 3-D volume into ``N`` consecutive 2-D sections, hold
out every other one, and reconstruct them. Most candidate datasets cannot support
...
``markers``       how many of the cortex marker/layer genes the panel carries.
...
The probe uses the evaluators' own ``_morans_i`` and ``_rank_normalize`` (via
``evaluate_paper``), so it is the same definition ``paper_morans_pearson`` uses.

Usage:
    python -m src.bench3.survey_datasets
    python -m src.bench3.survey_datasets --root /path/to/benchmark
...
def profile(name, path, n_sections=N_SECTIONS, probe=True, rng=None):
    ...
    try:
        ...
        # The partition: even split of the *planes*, then flatten.
...
def main():
    ap = argparse.ArgumentParser(
        description="Screen processed source datasets for the section protocol")
    ap.add_argument("--root", default=str(PROJECT_ROOT),
                    help="directory holding data/processed (default: benchmark/)")
