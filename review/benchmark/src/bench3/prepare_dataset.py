#!/usr/bin/env python
"""Build a paper-protocol dataset: a volume -> N consecutive 2-D sections.
...
def extract_xyz(adata, prefer="auto"):
    ...
    Accepts a raw volume or a *processed* file (``data/processed/``). Getting the units wrong is a
    ...
    Priority mirrors the processors in ``src/data/process/``: ``obs['x','y','z']`` first (the STARmap
    distribution carries the z-planes there even when ``obsm['spatial']`` is only
    2-D, and the processor leaves those columns as original voxel indices), then a 3-D obsm,
...
def cell_type_series(adata):
    """Cell-type labels, matching the processors' choice for comparability."""
...
def build(dataset=DATASET_NAME, raw_path=None, output_path=None, flatten_z=True,
    ...
    if not raw_path.exists():
        ...
        raise FileNotFoundError(
            ...
            f"  A *processed* data.h5ad (data/processed/) is an accepted source."
        )
    ...
    # A spec may state the source units outright. The ExSeq processor writes
...
def main():
    ap = argparse.ArgumentParser(description="Build a paper-protocol dataset")
    ap.add_argument("--dataset", default=DATASET_NAME,
                    help="dataset name from config.DATASET_SPECS")
    ap.add_argument("--raw", default=None, help="source h5ad (raw or processed)")
