"""Run one method against one holdout design.
...
Per (design, method) this orchestrator:
  ...
  4. Invokes the method wrapper (``src/bench3/methods/``) in generation-only mode with
  ...
  5. Evaluates the result three ways and merges them into one ``metrics.json``:
       ...
       * ``evaluate_generation``  — correspondence-free generation metrics
       * ``evaluate``             — cell-matched metrics, reference only

Usage:
    python -m src.bench3.run_benchmark --method spatialz
    python -m src.bench3.run_benchmark --method spatialcpav18_gen --design loo
"""
...
def dataset_meta(path):
    ...
    The benchmark handles more than one dataset, and the results tree, the log lines and the
...
def evaluate_prediction(prediction_path, dataset_path=DATASET_PATH,
    ...
    # the correspondence-free generation metrics (``src/benchmark/evaluate_generation.py``).
    ...
    # cell-matched metrics — reference only (no correspondence exists).
    try:
...
def run_single(method, holdout_config, dataset_path=DATASET_PATH,
    ...
    if not info.get("available", False):
        ...
        print(f"[bench] SKIP {method}: {reason}")
    ...
    print(f"[bench] {method} | {dataset_name} | {holdout_id}")
    ...
    if not wrapper.exists():
        return {"success": False,
                "error": f"wrapper not found: {wrapper} (is src/bench3/methods/ complete?)"}

    # REVIEW COPY: $BENCH_V3_PYTHON, when set, names the interpreter every wrapper
    # runs under instead of `conda run -n <conda_env> python`. Launcher only — the
    # wrapper, its arguments and everything it does are unchanged. Unset (the
    # default) is exactly the published behaviour. Meant for the one-environment
    # CPU lock (envs/lock/cpu-verified.txt); it cannot give spatialz/feast/isost
    # their own environments, so use it for v18-hosted methods only.
    launcher = (["conda", "run", "-n", info["conda_env"], "--no-capture-output", "python"]
                if not os.environ.get("BENCH_V3_PYTHON")
                else [os.environ["BENCH_V3_PYTHON"]])
    cmd = [
    ...
    # cwd is the benchmark root (``benchmark/``), so the wrapper's relative
    # imports and package discovery behave the same for every method.
...
def main():
    ap = argparse.ArgumentParser(
        description="Run one method against the STARmap paper design")
