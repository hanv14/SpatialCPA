...
``--include-gen`` adds the correspondence-free ``gen_*`` generation metrics as a
fifth group, which is useful for checking that the two metric families tell the
same story.
...
def main():
    ...
    ap.add_argument("--include-gen", action="store_true",
                    help="add the gen_* generation metrics as a fifth criterion")
