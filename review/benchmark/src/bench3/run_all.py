"""Run the full campaign: every method through the paper design.

By default this is the four-method comparison of this review copy —
SpatialZ, FEAST, isoST and spatialcpav18_gen — each reconstructing sections 2, 4
and 6 of the STARmap visual cortex from sections 1, 3, 5 and 7. The v18
expression-ablation rows (``v18_*``) are in METHOD_ORDER too and run by default;
name ``--methods`` to run a subset.

The training-only input is built once and shared by every method, so the
comparison is apples-to-apples by construction.

Usage:
    python -m src.bench3.run_all                              # every registered method
...
def main():
    ap = argparse.ArgumentParser(description="Run the full benchmark campaign")
    ...
    print(f"benchmark campaign — design={args.design}, dataset={dataset_name} "
