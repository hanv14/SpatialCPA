"""One row per *built* dataset: geometry, size, annotation, processing.

``survey_datasets`` screens candidate volumes in ``data/processed/`` for protocol
...
def main():
    ap = argparse.ArgumentParser(
        description="Summarize the built paper-protocol datasets as one CSV row each")
