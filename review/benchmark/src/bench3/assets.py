"""Method assets prepared before handing them to a wrapper.
...
The harness fixes it on its own side of the boundary: before the wrapper runs, the
...
method is unchanged in every way that matters — the harness just hands it an asset the
current torch will load. Neither the method wrappers nor the method packages are
touched.
