"""Read a dataset's source volume into an AnnData the benchmark can partition.

Most sources are already an ``.h5ad`` — a raw distribution or one of
``data/processed/`` files — and need no help. ExSeq is not: its raw
distribution is a cell-by-gene CSV plus a separate annotation file, and the
processed h5ad (also accepted) is produced from it by
``src/data/process/process_exseq_visual_cortex.py``.

``read_exseq_csv`` reads that raw form directly, mirroring what the processor
does, so a dataset can be built from ``data/raw/`` without the processing script
having been run first. It is a re-implementation, not a call into the processor,
and the two are kept equivalent on the points
...
def read_exseq_csv(path, verbose=True):
    ...
    if not csv_path.exists():
        raise FileNotFoundError(
            ...
            f"    data/raw/exseq_visual_cortex/{EXSEQ_CSV}")
    ...
    # Cell types live in the SpaceTx EDV results, row-aligned with the CSV. The processor
...
def read_imc_zstack(path, verbose=True):
    ...
    serial section at 10 um spacing. Mirrors the processor: z from the file index
...
def read_deep_starmap_csv(path, verbose=True):
    ...
    annotation, and the z value doubles as the section label — exactly what the
    processor does.
    """
    import anndata as ad
    import pandas as pd
...
def read_cosmx_raw(path, verbose=True):
    ...
    obs_names — so the two have to be joined. Mirrors the processor: pixels to um
...
def read_merfish_hypothalamus_csv(path, animal_id=MOFFITT_DEFAULT_ANIMAL,
    ...
    Mirrors the processor: Centroid_X/Y are already micrometres, z is Bregma in
...
# Every spelling the processor's ``find_ccf_columns`` tries, in its order. Keeping only the
    ...
    try:
        ...
        raise OSError(
            ...
            + f"  Re-download it, then rebuild:\n"
              f"    python src/data/download/download_allen_merfish_brain.py\n"
...
def read_allen_ccf(path, verbose=True, region=None):
    ...
    * ``Zhuang-ABCA-N``          — ``<region>-log2.h5ad`` plus
    ...
    Mirrors ``src/data/process/process_allen_*.py`` rather than
    calling it, so the benchmark stays self-contained — including the mm -> um conversion,
    which is the one step that is silently wrong if skipped (CCF coordinates are
    millimetres and every other dataset here is micrometres).
...
def load_source(path, spec, verbose=True):
    ...
    path when it points at one, so a raw directory and a processed file are
