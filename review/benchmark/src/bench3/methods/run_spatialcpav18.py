"""SpatialCPA-v18 — benchmark wrapper.
...
# The shared wrapper I/O (``_v2_io``, beside this file) and leakage guard
# (``src/benchmark/leakage_guard.py``) — the same two modules every wrapper uses,
# which is what keeps all methods' rows comparable.
_HERE = Path(__file__).resolve().parent                    # src/bench3/methods
sys.path.insert(0, str(_HERE))                             # _v2_io
sys.path.insert(0, str(_HERE.parents[1] / "benchmark"))    # leakage_guard
import _v2_io                      # noqa: E402
import leakage_guard               # noqa: E402


# The v18 method is a single file, not a package. Resolution order:
#   1. $SPATIALCPAV18_FILE — the file itself;
#   2. $SPATIALCPAV18_ROOT — the directory that contains it;
#   3. <repo root>/learn_spatialcpav18.py, where <repo root> is the directory
#      holding benchmark/ (parents[4] of this file). Nothing else.
#
# REVIEW COPY: the published wrapper walked *every* parent directory looking for
# the file. That works in a fresh clone only by luck — it keeps climbing past the
# repository, so with the committed copy missing it would silently load whatever
# learn_spatialcpav18.py sits in some enclosing directory (in the parent project,
# a different checkout's copy). The walk now stops at the repository root, and the
# loaded file's sha256 is printed in the method log so a run states which v18 it
# ran. Nothing about how the module is used changed.
FILE_ENV = "SPATIALCPAV18_FILE"      # full path to learn_spatialcpav18.py
ROOT_ENV = "SPATIALCPAV18_ROOT"      # directory that CONTAINS the file
_MODULE_NAME = "learn_spatialcpav18"
_FILENAME = "learn_spatialcpav18.py"
_REPO_ROOT = Path(__file__).resolve().parents[4]


def _candidate_files():
    """Paths that might be ``learn_spatialcpav18.py``, most-specific first."""
    f = os.environ.get(FILE_ENV)
    if f:
        yield Path(f).expanduser()
        return                      # an explicit file never falls through
    r = os.environ.get(ROOT_ENV)
    if r:
        yield Path(r).expanduser() / _FILENAME
        return
    yield _REPO_ROOT / _FILENAME
...
def _load_module():
    """Import ``learn_spatialcpav18.py`` from disk under a stable module name.
...
_V18, _V18_PATH, _TRIED = _load_module()
...
def check_environment():
    if _V18 is None:
        ...
        if env_f:
            ...
            print(f"  Keep {_FILENAME} at the repository root ({_REPO_ROOT}), or "
                  f"set ${FILE_ENV} to the file, or ${ROOT_ENV} to the directory "
                  f"holding it.", file=sys.stderr)
        return False
    try:
        import torch
        t = f"torch {torch.__version__} ({'cuda' if torch.cuda.is_available() else 'cpu'})"
    except Exception:
        # v18 degrades to the numpy latent-grounded fallback without torch — a
    ...
    print(f"spatialcpav18 (learn_spatialcpav18.py) from {_V18_PATH}; {t}")
    print(f"learn_spatialcpav18.py sha256 {_sha256(_V18_PATH)}")
    return True
...
def build_stack(adata, X_log, X_raw, ct_all):
    ...
    the pre-normalization measurement carried alongside so v18's ``raw_output``
    can emit grounded cells' real profiles verbatim (see ``learn_spatialcpav18``
    ``main()``, which does the same with ``X_raw=vol.X``).
    """
    Slice, SliceStack = _V18.Slice, _V18.SliceStack
...
def main():
    ...
    # Capture the RAW measurement before normalization (v18 raw_output emits it
    ...
    print(f"Running SpatialCPA-v18 for targets "
