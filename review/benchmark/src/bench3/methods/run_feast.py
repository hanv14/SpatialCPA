...
def main():
    """Generation-only entry point (reuses FEAST run_method)."""
    parser = argparse.ArgumentParser(
        description="FEAST virtual slice interpolation")
    ...
    print(f"Running FEAST (generation-only) for {[(s, round(z, 2)) for s, z in targets]}...")
    t0 = time.time()
    results = run_method(train_adata, target_z, seed=args.seed,
                         paste2_s=args.paste2_s, paste2_alpha=args.paste2_alpha)
    wall_time = time.time() - t0

    method_params = {"seed": args.seed, "paste2_s": args.paste2_s,
                     "paste2_alpha": args.paste2_alpha, "generation_only": True}
