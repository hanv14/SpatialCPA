"""Shared I/O and guards for the generation-only method wrappers.

Every method wrapper shares the same contract:
...
slices and writes ``prediction.h5`` in the standard benchmark format (so the
evaluators read it unchanged).  Crucially the input
