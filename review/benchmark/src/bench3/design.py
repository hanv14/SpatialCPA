"""Holdout designs for the benchmark.

The **paper design** is the default and the one the whole benchmark exists for:
...
A design is a JSON-serializable dict with the same keys the original ``run_benchmark``
...
def main():
    ap = argparse.ArgumentParser(description="Emit holdout designs")
