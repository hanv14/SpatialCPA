#!/usr/bin/env python
"""Download 3D IMC breast cancer dataset (Kuett et al. 2022).

Source: Figshare
Target: data/raw/imc_breast_cancer/
...
DATASET_NAME = "imc_breast_cancer"
BASE_DIR = Path(__file__).resolve().parents[3] / "data" / "raw" / DATASET_NAME

# 15 h5ad files, one per z-step
FIGSHARE_IDS = [
    58960081, 58960084, 58960078, 58960093, 58960087,
    58960090, 58960096, 58960099, 58960105, 58960111,
    58960117, 58960114, 58960102, 58960108, 58960120,
]

FILES = [
    {
        "url": f"https://ndownloader.figshare.com/files/{fid}",
        "name": f"MainHer2BreastCancerModel_zstep10_{i}.h5ad",
        "size": None,
    }
    for i, fid in enumerate(FIGSHARE_IDS)
]
...
def verify():
    ...
    # h5ad content checks on first file
    try:
        import anndata
        first_file = BASE_DIR / FILES[0]["name"]
        if first_file.exists():
            ...
            print(f"  h5ad OK (sample): {adata.n_obs} cells, {adata.n_vars} genes")
...
def main():
    print(f"Downloading {DATASET_NAME} (15 z-step h5ad files)...")
