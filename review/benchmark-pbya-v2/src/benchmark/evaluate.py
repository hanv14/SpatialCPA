...
def align_predictions_inplace(pred, gt_adata):
    """Rigidly align predicted (x, y) onto the GT frame, per held-out section.

    v2 methods synthesize in the training frame, which — after training-only
    re-registration — differs from the held-out GT frame by a rigid (+scale)
    transform. We recover that transform per section from the two point clouds
    (an evaluation-side operation that uses the GT, never fed back to the method)
    and overwrite ``pred['x']/pred['y']`` so every downstream spatial metric is
    computed in a common frame. Returns the number of sections aligned.
    """
    gt_sections = gt_adata.obs["section"].values.astype(str)
    gt_spatial = gt_adata.obsm["spatial"]
    n_aligned = 0
    for sec in np.unique(gt_sections):
        pmask = pred["section"] == sec
        gmask = gt_sections == sec
        if pmask.sum() < 3 or gmask.sum() < 3:
            continue
        pred_xy = np.column_stack([pred["x"][pmask], pred["y"][pmask]])
        gt_xy = gt_spatial[gmask, :2]
        aligned = align_prediction_to_gt(pred_xy, gt_xy, with_scale=True)
        pred["x"][pmask] = aligned[:, 0]
        pred["y"][pmask] = aligned[:, 1]
        n_aligned += 1
    return n_aligned
...
def evaluate(prediction_path, h5ad_path, output_path=None):
    ...
    # v2: align synthesized coords onto the GT frame before any spatial metric,
    # so the (arbitrary, re-registered training) frame doesn't penalise methods.
    n_aligned = align_predictions_inplace(pred, gt)
    ...
    metrics = {
        ...
        "eval_aligned_sections": int(n_aligned),
    }
