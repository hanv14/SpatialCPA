...
# v2 shares the downloaded SpatialZ tool with v1.
TOOLS_DIR = (Path(__file__).resolve().parents[4] / "benchmark-pbya"
             / "tools" / "spatialz" / "SpatialZ_code")
...
def main():
    """benchmark-pbya-v2 generation-only entry point.

    The input is training-only and already re-registered; the held-out (x, y) are
    never provided — only a scalar target z per section. SpatialZ's synthesis
    (`run_method`) already operates on training slices + a section->z map and
    finds its own flanking slices, so it is reused unchanged.
    """
    parser = argparse.ArgumentParser(
        description="SpatialZ virtual slice generation (benchmark-pbya-v2)")
    ...
    print(f"Running SpatialZ (generation-only) for {[(s, round(z, 2)) for s, z in targets]}...")
    t0 = time.time()
    results = run_method(train_adata, target_z, seed=args.seed,
                         syn_mode=args.syn_mode, device=args.device)
    wall_time = time.time() - t0

    method_params = {"seed": args.seed, "syn_mode": args.syn_mode,
