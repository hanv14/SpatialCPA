...
_V18_WRAPPERS = ("run_spatialcpav18.py", "run_spatialcpav18_ml.py")
...
METHODS = {
    "spatialcpav18_gen": {
        ...
        # and the flags arrived as ``run_all ... -- <flags>`` extras. Pinning them
        # makes a bare ``run_all --methods spatialcpav18_gen`` reproduce the
        # published configuration, and stops run_all forwarding v18-only flags
        # to spatialz/feast/isost when they run in the same invocation.
        #
...
# Every v18-hosted method (spatialcpav18_gen and the v18_* expression ablations)
# runs under V18_ARGS. Prepended, so an entry's own flags (``--learner`` …) and any
# command-line extras still come after it and win on a conflict.
for _m in METHODS.values():
...
METHOD_ORDER = [
    ...
    "spatialcpav18_gen",
    # v18 expression ablation (see METHODS): v18 in every respect but the step
    # that emits expression.
    "v18_ridge", "v18_lasso", "v18_knn", "v18_rf",
    "v18_gbm", "v18_xgb", "v18_lgbm", "v18_mlp", "v18_tabm",
    "v18_gbmfield", "v18_gbmrepair", "v18_lgbmfield", "v18_lgbmrepair",
    "v18_lgbmband", "v18_lgbmbalance",
]
