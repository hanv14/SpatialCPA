...
class SpatialCPAv14:
    ...
    def _phase_b(self):
        ...
        for ep in range(cfg.epochs):
            ...
            for pl in plan:
                ...
                pool_h = torch.cat([self.S[j]["h"] for j in pl["src"]], 0)
                drop = None
                if rng.random() < cfg.gap_dropout and len(pl["src"]) > 1:
                    drop = int(rng.integers(len(pl["src"])))       # GAP-AWARE dropout
                ctx = self._context(pool_h, pl["pool_nxy"], pl["pool_z"], pl["pool_owner"],
                                    pl["src"], self.S, query_nxy, zq, self.ctxmod, self.dev,
                                    drop_owner=drop)
    ...
    def _generate(self, z):
        ...
        expr, ct_idx = self._ground(anchor, anchor_src, e_hat_np, pool_nxy, pool_e,
                                    pool_expr, pool_type, rng)
        if cfg.composition_match and self.n_types >= 2:
            ct_idx, expr = self._match_composition(lower, upper, t, ct_idx, expr, anchor,
                                                   pool_nxy, pool_e, e_hat_np, pool_type,
                                                   pool_expr, rng)
        ...
        if cfg.output_counts:                                    # the evaluator wants counts
    ...
    def _ground(self, anchor, anchor_src, e_hat, pool_nxy, pool_e, pool_expr, pool_type, rng):
        ...
        if n_flow > 0:
            ...
            for r, i in enumerate(sel):
                ci = cand[r]
                d = np.linalg.norm(pool_e[ci] - e_hat[i], axis=1)  # nearest in flow-latent space
                pick[i] = ci[int(np.argmin(d))]
        expr[:] = pool_expr[pick]
        ct = pool_type[pick]
        return expr, (ct if self.n_types >= 2 else None)
