"""SpatialCPA-v5 (transformer) — benchmark-pbya-v2 generation-only wrapper.
...
The ``spatialcpav5`` package is imported from the repository root; set
SPATIALCPAV5_ROOT to override auto-discovery.
"""
...
# ── Locate the spatialcpav5 source package ────────────────────────────────────

def _add_spatialcpav5_to_path():
    candidates = []
    env_root = os.environ.get("SPATIALCPAV5_ROOT")
    if env_root:
        candidates.append(Path(env_root))
    candidates.extend(Path(__file__).resolve().parents)
    for cand in candidates:
        if (cand / "spatialcpav5" / "__init__.py").exists():
...
_SPATIALCPAV5_ROOT = _add_spatialcpav5_to_path()
...
def check_environment():
    try:
        ...
        return False
    if _SPATIALCPAV5_ROOT is None:
        print("ERROR: could not locate the `spatialcpav5` package.", file=sys.stderr)
        return False
    try:
        import torch
        from spatialcpav5 import SpatialCPATransformer, Trainer, Predictor  # noqa: F401
        print(f"spatialcpav5 imported from {_SPATIALCPAV5_ROOT}, "
              f"CUDA: {torch.cuda.is_available()}")
        return True
    except ImportError as e:
        print(f"ERROR: failed to import spatialcpav5: {e}", file=sys.stderr)
        return False
...
def build_slices(adata, cell_type_names, region_names, ct_all, reg_all):
    """Build spatialcpav5 Slice objects from the training-only input."""
    from spatialcpav5 import Slice
...
def run_method(adata, targets, gene_names, args):
    ...
    from spatialcpav5 import (
        SliceStack, SpatialCPATransformer, SpatialCPAv5Config, Trainer,
    ...
    cfg.train.checkpoint_dir = str(Path(args.output).parent / "spatialcpav5_ckpt")
    ...
    cfg.inference.celltype_source = args.celltype_source
    ...
    for sec, z in targets:
        ...
        try:
            vs = predictor.generate_virtual_slice(
                ...
                celltype_source=cfg.inference.celltype_source)
...
def main():
    parser = argparse.ArgumentParser(
        description="SpatialCPA-v5 generation-only wrapper (benchmark-pbya-v2)")
    ...
    parser.add_argument("--position-source", default="interpolate",
                        choices=["interpolate", "density", "flanking", "grid"],
                        help="candidate cell positions: interpolate (v5 default — "
                             "match flanking cells and place one cell per pair at "
                             "the z-interpolated position; count = interpolated "
                             "flanking count), density (sample the predicted density "
                             "field), flanking (both neighbor slices stacked), or "
                             "grid (uniform lattice)")
    parser.add_argument("--celltype-source", default="transfer",
                        choices=["transfer", "predict"],
                        help="cell-type annotation: transfer (v5 default — copy the "
                             "nearest training cell's type, consistent with the "
                             "transferred expression) or predict (label-head argmax)")
    ...
    print(f"Running SpatialCPA-v5 (generation-only) for targets "
    ...
    method_params = {
        ...
        "celltype_source": args.celltype_source,
