...
|---------|--------|-----|--------|-------|
| `run_spatialcpav15.py` | SpatialCPA-v15 (gen) | bench_spatialcpa | available | **Four-phase generative atlas pipeline** (`spatialcpa_v15_idea.md`): continuous hierarchical **type embedding** from signed marker effect sizes (data backbone + encoder prior, blended per type by sample size) → multi-channel **structure field** completed by a *flow-free, query-based* conditional diffusion `(I_{a−1}, I_a, I_b, I_{b+1}, t, Δz) → I_t` with deterministic DDIM multi-query and a **machine-checked stop condition** vs linear interpolation → layout as a **marked spatial point process** → **NB expression VAE + conditional latent diffusion** with neighbourhood cross-attention over the real bracketing slices → decode + **mandatory per-cell provenance** (distance to nearest real slice, cross-seed spread, sub-Nyquist flag). Shares no machinery with v8 (no OT plan, no morph, no niche MRF). PyTorch. See `spatialcpav15/README.md`. |
| `run_spatialcpav11.py` | SpatialCPA-v11 (gen) | bench_spatialcpa | available | **Two-stage continuous 3D neural field**: Stage-1 LayoutField (positions+type, distilled from an OmiCLIP/Path2Space teacher + self-supervision) → Stage-2 ExpressionField (layout-conditioned). Continuous z-query (between/beyond slices) with z-marginalization; cross-z consistency + biology constraints. PyTorch; falls back to nearest-slice. See `spatialcpav11/README.md`. |
