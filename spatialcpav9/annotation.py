"""
Cell-type annotation for SpatialCPA-v9.
...
2. **Communication / niche** — labels are then refined by the MRF in
   :mod:`spatialcpav9.communication` so the spatial co-occurrence of types matches
...
def class_logprior(query_embed, train_embed, train_labels, n_types, cfg):
    ...
    ``cfg`` is an :class:`~spatialcpav9.config.AnnotationConfig`.
