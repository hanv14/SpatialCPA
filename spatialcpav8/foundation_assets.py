"""
Foundation-model gene-embedding assets for SpatialCPA-v8.

v8 injects an external biological prior by projecting expression through a
pretrained *gene embedding* ``W`` (``cell = X_norm @ W``; see
:mod:`spatialcpav8.embedding`, method ``"fm_gene"``/``"concat"``). This module
converts the gene-embedding tables shipped by common spatial-omics / single-cell
foundation models into the ``.npz`` format v8 expects — arrays ``genes`` (str,
...
CLI::
    ...
    python -m spatialcpav8.foundation_assets --source gene2vec \\
        --input gene2vec_dim_200.txt --output gene_emb.npz
    python -m spatialcpav8.foundation_assets --source scgpt \\
...
def save_npz(genes: Sequence[str], embedding: np.ndarray, out_path: str) -> None:
    """Write a gene-embedding table in v8's ``.npz`` format."""
...
def main():
    ap = argparse.ArgumentParser(
        description="Convert a pretrained gene embedding into SpatialCPA-v8 .npz")
