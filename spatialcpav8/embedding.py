"""
Cell-state embeddings for SpatialCPA-v8.
...
def _load_gene_embedding(path: Optional[str], gene_names, dim: int):
    ...
    """
    path = path or os.environ.get("SPATIALCPAV8_FM_GENE_EMBEDDING")
...
def build_embedder(train_expression: np.ndarray, gene_names, cfg) -> Embedder:
    ...
    ``cfg`` is an :class:`~spatialcpav8.config.EmbeddingConfig`.
    """
    method = cfg.method
    ...
    if method in EMBEDDER_REGISTRY:
        try:
            ...
            print(f"[spatialcpav8.embedding] registered embedder '{method}' failed "
    ...
    if method == "fm_gene":
        fn = _make_fm_gene()
        if fn is None:
            print("[spatialcpav8.embedding] no gene-embedding matrix found "
                  "(fm_gene_embedding_path / SPATIALCPAV8_FM_GENE_EMBEDDING); "
                  "falling back to PCA.")
    ...
    if method == "concat":
        ...
        fm_fn = _make_fm_gene()
        if fm_fn is None:
            print("[spatialcpav8.embedding] concat: gene-embedding matrix missing; "
                  "using PCA only.")
