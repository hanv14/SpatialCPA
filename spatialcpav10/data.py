"""
Lightweight data containers for SpatialCPA-v10.
...
These mirror the v5 structures but are self-contained so v8 has no import
dependency on the v5 package (the two coexist and can be benchmarked side by
side). All coordinates are physical ``(x, y, z)``; the flanking slices handed to
the synthesizer are the *training-only, re-registered* sections — the held-out
slice never enters this stack (enforced upstream by the benchmark).
"""
