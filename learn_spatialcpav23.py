...
r"""
================================================================================
 learn_spatialcpav23.py   (v20 + MODULE CHIMERISM / STRATIFIED MIX / LOCAL
                           COMPOSITION FIELDS / PAIRED POSITION MORPH)
...
V23 CHANGES (each keyed to one benchmark metric family; all gated so that at
narrow gaps with the features off, v23 == v20 == v18 exactly)
-----------------------------------------------------------------------------
 DESIGN PRINCIPLE.  SpatialZ's expression synthesis is PER-GENE chimerism:
 every gene of a generated cell is sampled INDEPENDENTLY from up to k_sam=3
 same-type spatial neighbours (niche-cosine weighted, NO z-term).  Independent
 per-gene draws maximise embedding novelty (its one reliably winning metric)
 but factorise the joint distribution gene-by-gene: gene-gene covariance,
 co-expression modules, and the per-cell count structure are destroyed by
 construction.  v20 sat at the opposite pole (whole-profile copies + a small
 per-gene mix).  v23's contribution is the PRINCIPLED MIDDLE: chimerism at the
 granularity of CO-EXPRESSION MODULES - the coarsest unit that still creates
 combinatorial novelty, and the finest unit that cannot break within-module
 covariance.  Everything below follows from that one idea plus variance
 reduction and locality.

 1. MODULE-LEVEL CHIMERISM (cfg.module_mix, cfg.n_gene_modules):
    Gene co-expression modules are estimated ONCE from the training data
    (genes clustered by their PCA loading vectors - no extra dependency, no
    G x G matrix).  Both the v18 gene-mix and the v20 Bernoulli cross-mix now
    operate per (cell, MODULE): a whole module is taken from one real partner.
      -> embedding mixing rises (novel module combinations, like SpatialZ)
      -> Geary's C / Moran's I / co-expression survive (within-module
         covariance is copied intact, unlike SpatialZ).
    Targets: paper_umap_mixing, paper_gearys_pearson, paper_morans_pearson.
 2. NICHE-WEIGHTED PARTNER SELECTION (cfg.niche_weight):
    Mix/cross partners are weighted by cosine similarity of the cells'
    morphology (soft local type-composition + density) vectors - the same
    quantity SpatialZ buys with MENDER, computed here from channels we
    already have.  Partners share niche, so module swaps stay biologically
    local.  Targets: marker fields, co-expression, nhood metrics.
 3. STRATIFIED (VARIANCE-REDUCED) CROSS-MIX (cfg.cross_mix_stratified):
    v20 drew the Bernoulli mask iid per cell.  v23 draws EXACTLY
    round(w * n_group) cells per (module, flank-group) - the same marginal
    expectation with zero proportion variance, so binned expression FIELDS at
    intermediate z are far less noisy.  Targets: paper_marker_field_r,
    paper_marker_depth_r, paper_morans_pearson at wide gaps.
 4. LOCAL COMPOSITION-FIELD MATCHING (cfg.composition_local):
    v14-v20 matched the GLOBAL type histogram to the interpolated flank
    histogram.  v23 matches the local composition FIELD: the plane is binned,
    each bin's target composition is the t-interpolation of the flanks'
    composition in that bin, and surplus->deficit re-grounding runs per bin.
    Global marginals still match (sum of bins), but type territories now sit
    where the interpolated tissue puts them.
    Targets: paper_celltype_localization, nhood enrichment.
 5. PAIRED POSITION MORPH (cfg.pair_morph, cfg.pair_morph_strength):
    At wide gaps tissue structures DRIFT between flanks; resampling flank
    coordinates verbatim leaves every structure at its source-flank position
    (a half-gap positional error, the geometric twin of the molecular error
    v19/v20 fixed).  v23 nudges each generated cell along the vector from its
    exemplar to its cross-flank same-type partner by the flank fraction
    (displacement capped at a few cell spacings).  This is CORRESPONDENCE
    morphing - SpatialZ's SWD polish matches only the marginal point cloud
    and cannot move a structure.  Gated by alpha: OFF at narrow gaps.
    Targets: paper_marker_field_r, paper_celltype_localization at wide gaps.
 6. HYGIENE (correctness of the comparison itself):
    - rank_normalize now uses AVERAGE ranks for ties (mergesort ranks gave
      the zero-mass of sparse genes arbitrary rank spread);
    - gene_mean/var "spearman" metrics now actually compute Spearman;
    - marker FIELD r and CELL-TYPE LOCALIZATION metrics added to the
      teaching evaluator (they were benchmark-only before);
    - generation RNG is seeded per (seed, z) so held-out sections are
      independent draws;
    - raw output gates on whether a decode blend was ACTUALLY applied, not
      on the config value;
    - --holdout alternating|consecutive and the benchmark config knobs are
      exposed on the CLI (defaults reproduce the benchmark flags).
...
can execute it with no data download.
    ...
    $ python learn_spatialcpav23.py            # full torch pipeline + eval
    $ python learn_spatialcpav23.py --fast     # tiny + quick (smoke test)
    $ python learn_spatialcpav23.py --h5ad data/starmap/STARmap_...3D_data.h5ad
...
def fit_gene_modules(components: np.ndarray, n_modules: int, seed: int = 0) -> np.ndarray:
    r"""v23: cluster genes into CO-EXPRESSION MODULES from their PCA loadings.

    Column g of `components` (d, G) is gene g's loading vector: genes that
    co-vary across the training cells have similar (up to sign+scale) loading
    directions, so clustering the UNIT-NORMALIZED loading vectors recovers
    co-expression modules without ever forming a G x G correlation matrix and
    without any dependency beyond sklearn (with a quantile fallback).

    Why modules matter: they are the granularity at which chimerism is safe.
    Swapping a whole module between two same-type, same-niche cells creates a
    novel combination (embedding mixing goes UP) while the covariance INSIDE
    the module - the part the co-expression / Geary metrics actually measure -
    is copied intact.  Per-gene swaps (SpatialZ) destroy it; whole-profile
    copies (v14) create nothing novel.  Returns (G,) int module ids."""
    C = np.asarray(components, np.float64)                      # (d, G)
    G = C.shape[1]
    m = int(max(1, min(n_modules, G)))
    if m == 1 or G <= 2:
        return np.zeros(G, np.int64)
    V = C / (np.linalg.norm(C, axis=0, keepdims=True) + 1e-12)  # unit loading dirs
    # canonicalize sign (flip each gene so its largest-|.| component is +):
    # ANTI-correlated genes carry covariance too, and their loading vectors are
    # near-opposite; without the flip they land in different modules and their
    # covariance is broken by a module swap.
    flip = np.sign(V[np.abs(V).argmax(0), np.arange(V.shape[1])])
    V = V * np.where(flip == 0, 1.0, flip)[None, :]
    try:
        from sklearn.cluster import KMeans
        km = KMeans(n_clusters=m, n_init=10, random_state=seed).fit(V.T)
        lab = km.labels_.astype(np.int64)
    except Exception:                                           # quantile fallback
        lab = (np.argsort(np.argsort(V[0])) * m // G).astype(np.int64)
    # guard: no empty modules (re-map labels densely)
...
class V14Config:
    ...
    curriculum_flow: bool = False         # v20: default OFF - ablation shows it
                                          ...
                                          # training time.  When ON: with prob
                                          # curriculum_p the Phase-B context
                                          # EXCLUDES the nearest slice each side
                                          # (long-range conditioning training).
    curriculum_p: float = 0.35
    # --- v23 additions -------------------------------------------------------
    module_mix: bool = True               # chimerism at CO-EXPRESSION-MODULE
                                          # granularity: gene-mix picks one
                                          # partner PER (cell, module) and the
                                          # cross-mix Bernoulli mask is drawn
                                          # per (cell, module).  Novel module
                                          # combinations (mixing goes up) with
                                          # within-module covariance copied
                                          # intact (Geary/Moran/co-expr safe).
                                          # False = v20's per-gene behaviour.
    n_gene_modules: int = 8               # modules from KMeans on the genes'
                                          # PCA loading vectors (unit-normed);
                                          # clipped to the number of genes.
    niche_weight: float = 1.0             # weight of morphology-vector cosine
                                          # similarity when sampling mix/cross
                                          # partners (0 = v20's uniform/latent-
                                          # only choice).  The internal
                                          # equivalent of SpatialZ's MENDER
                                          # niche weighting.
    cross_mix_stratified: bool = True     # draw EXACTLY round(w * n) partner-
                                          # valued cells per (module, flank
                                          # group) instead of iid Bernoulli:
                                          # identical expectation, zero
                                          # proportion variance -> cleaner
                                          # binned fields at intermediate z.
    composition_local: bool = True        # match the local composition FIELD
                                          # (per spatial bin, t-interpolated
                                          # from the flanks) instead of only
                                          # the global histogram.
    comp_bins: int = 6                    # composition grid is comp_bins^2
                                          # over the occupied plane (bins with
                                          # < comp_min_cells cells fall back
                                          # to the global pass).
    comp_min_cells: int = 25
    pair_morph: bool = True               # move each generated cell along the
                                          # exemplar -> cross-flank-partner
                                          # vector by the flank fraction
                                          # (correspondence morphing of tissue
                                          # drift).  Gated by alpha, so OFF at
                                          # narrow gaps.
    pair_morph_strength: float = 1.0      # 0..1 scale on the morph vector
    pair_morph_cap: float = 3.0           # cap the displacement at this many
                                          # median cell spacings (safety)
    # -------------------------------------------------------------------------
...
class SpatialCPAv14:
    ...
    def __init__(self, stack: SliceStack, gene_names: Sequence[str],
        ...
        # ---- v23: gene co-expression modules (training-only, fit once) ----
        self.gene_modules = fit_gene_modules(self.expr_latent.components_,
                                             self.cfg.n_gene_modules, self.cfg.seed)
        self.n_modules = int(self.gene_modules.max()) + 1
    ...
    def _generate(self, z):
        ...
        # v23: seed per (seed, z) so different held-out sections are independent
        # draws (v20 reused one stream: identical patch fields / Bernoulli masks).
        ...
        # v23: standardized morphology (soft local type composition + density)
        # per pool cell - the niche vector used to weight mix/cross partners.
        pool_m = np.vstack([lo_st["m"].cpu().numpy(), hi_st["m"].cpu().numpy()])
        ...
        if cfg.composition_match and self.n_types >= 2:
            if cfg.composition_local:
                # v23: match the local composition FIELD (per spatial bin),
                # then let the global pass mop up any residual imbalance.
                ct_idx, expr, pick = self._match_composition_local(
                    lower, upper, t, ct_idx, expr, anchor, pool_nxy, pool_e,
                    e_hat_np, pool_type, pool_expr, rng, pick, n_lo)
        ...
        blended = False                                # v23: track ACTUAL blending
        ...
        if cfg.cross_mix and alpha > 0.0:
            partner2, ok2 = self._pair_across_flanks(anchor, pick, ct_idx, n_lo,
                                                     ...
                                                     pool_m=pool_m)
            # v20 BERNOULLI CROSS-MIX: take the cross-flank partner's REAL value
            # with probability alpha * w_other (w_other = fraction of the
            # opposite flank at this cell's z).  E[output] equals linear
            # interpolation, but no value is ever synthesized, so detection
            # frequency / sparsity match real data by construction.
            # v23 refinements on top:
            #   * MODULE granularity (cfg.module_mix): the Bernoulli unit is a
            #     co-expression module, not a gene - within-module covariance
            #     is copied intact while module combinations stay novel.
            #   * STRATIFIED mask (cfg.cross_mix_stratified): exactly
            #     round(w * n) cells flip per (module, flank group) - same
            #     expectation, zero proportion variance, so binned fields at
            #     intermediate z are far less noisy.
            in_lower = pick < n_lo
            w_other = np.where(in_lower, t, 1.0 - t) * alpha
            cross_mask = self._mix_mask(w_other, ok2, in_lower, expr.shape[1], rng)
            expr = np.where(cross_mask, pool_expr[partner2], expr)

            # v23 PAIRED POSITION MORPH: structures drift between distant
            # flanks; move each cell along the exemplar -> cross-flank-partner
            # vector by its flank fraction (the positional twin of the
            # molecular cross-mix).  Correspondence-based, so it can MOVE a
            # structure - SpatialZ's SWD polish only reshapes the marginal
            # point cloud.  Capped at a few cell spacings; gated by alpha.
            if cfg.pair_morph and cfg.pair_morph_strength > 0.0:
                u = np.where(in_lower, t, 1.0 - t) * alpha * cfg.pair_morph_strength
                shift = (pool_nxy[partner2] - pool_nxy[pick]) * u[:, None]
                dsp_all, _ = cKDTree(pool_nxy).query(pool_nxy, k=2)
                sp_med = float(np.median(dsp_all[:, 1])) + 1e-9
                nrm = np.linalg.norm(shift, axis=1, keepdims=True) + 1e-12
                cap = cfg.pair_morph_cap * sp_med
                shift = shift * np.minimum(1.0, cap / nrm)
                shift[~ok2] = 0.0
                anchor = (anchor + shift).astype(np.float32)
        ...
        # ---- v18/v23: partial gene resampling for distributional novelty ----
        # v23: the resampling unit is a co-expression MODULE and each module
        # draws its OWN partner (niche-weighted), so one cell can combine
        # modules from several real neighbours - the combinatorial novelty that
        # made SpatialZ win the OT/mixing metrics - without ever breaking
        # within-module covariance the way SpatialZ's per-gene draws do.
        partner_pg = None                                 # (n, G) per-gene partner
        if cfg.gene_mix_frac > 0.0 and cfg.edit_weight == 0.0:
            partner_pg, gmask = self._gene_mix(anchor, pick, ct_idx, pool_expr,
                                               pool_nxy, pool_type,
                                               pool_expr.shape[1], rng,
                                               pool_m=pool_m)
            expr = self._apply_per_gene(expr, pool_expr, partner_pg, gmask)

        expr = np.clip(expr, 0.0, None)
        # ---- v18 output path -------------------------------------------------
        # When every emitted profile is a verbatim copy and the raw measurements
        # are available, emit the RAW profile of the picked source cell.  v14
        # instead emitted expm1(log-lib-normalized), which is NOT the inverse of
        # the wrapper normalization (per-cell totals were rescaled to the median
        # library size before log1p): on wide-dynamic-range data this nonlinear
        # round trip inverts the per-gene VARIANCE ranking (the negative
        # gene_var_spearman on all three EASI-FISH sets).
        # v23: gate on whether a decode blend was ACTUALLY applied (`blended`),
        # not on the config values - setting edit_gap_extra > 0 no longer
        # disables raw output at narrow gaps where the blend never fired.
        raw_ok = (cfg.raw_output and not blended
        ...
        if raw_ok:
            ...
            if partner_pg is not None:                   # gene/module-mix on raw scale
                expr = self._apply_per_gene(expr, pool_raw, partner_pg, gmask)
    ...
    def _pair_across_flanks(self, anchor, pick, ct_idx, n_lo, pool_nxy, pool_e,
        ...
        ok = np.zeros(n, bool)
        nw = float(getattr(cfg, "niche_weight", 0.0))
        use_niche = (pool_m is not None and nw > 0.0)
        if use_niche:                                # unit-normalized niche vecs
            pm = pool_m / (np.linalg.norm(pool_m, axis=1, keepdims=True) + 1e-9)
        for i in range(n):
            ...
            if use_niche:
                # v23: score = normalized latent distance MINUS niche cosine
                # similarity to the exemplar (the SpatialZ/MENDER idea, from
                # channels we already compute) - partners share both molecular
                # state and micro-environment, so the swapped values stay
                # locally plausible.
                dn = d / (np.median(d) + 1e-9)
                cs = pm[ci] @ pm[pick[i]]
                score = dn - nw * cs
            else:
                score = d
            partner[i] = ci[int(np.argmin(score))]
            ok[i] = True
        return partner, ok

    def _mix_units(self, n_genes):
        """v23: the chimerism unit ids per gene.  Module granularity when
        cfg.module_mix (co-expression modules from the PCA loadings), else one
        unit per gene (the v18/v20 behaviour)."""
        if self.cfg.module_mix and getattr(self, "gene_modules", None) is not None \
                and len(self.gene_modules) == n_genes:
            return self.gene_modules
        return np.arange(n_genes, dtype=np.int64)

    def _apply_per_gene(self, expr, pool, partner_pm, gmask):
        """Apply a per-(cell, unit) partner assignment on an arbitrary value
        scale (log pool at generation, raw pool on the raw-output path): for
        every unit (module or single gene), the genes of that unit are gathered
        from that cell's unit-partner where the gene mask is set."""
        units = self._mix_units(pool.shape[1])
        out = expr
        for m in range(partner_pm.shape[1]):
            cols = np.where(units == m)[0]
            if cols.size == 0:
                continue
            sub = pool[partner_pm[:, m]][:, cols]              # (n, |cols|)
            out[:, cols] = np.where(gmask[:, cols], sub, out[:, cols])
        return out

    def _mix_mask(self, w_other, ok2, in_lower, n_genes, rng):
        r"""v23: build the cross-mix mask at UNIT granularity (module or gene).

        STRATIFIED sampling (cfg.cross_mix_stratified): within each flank group
        (all lower-sourced cells share w = t*alpha; upper-sourced share
        (1-t)*alpha), exactly round(w * n_group) cells flip per unit, chosen
        uniformly at random.  The per-cell marginal equals the iid Bernoulli of
        v20, but the REALIZED proportion per unit is exact, so binned fields at
        intermediate z carry no extra sampling noise (a plain variance-reduction
        argument - antithetic to nothing, it just removes binomial jitter)."""
        cfg = self.cfg
        n = w_other.shape[0]
        units = self._mix_units(n_genes)
        M = int(units.max()) + 1 if n_genes else 0
        mask_u = np.zeros((n, M), bool)
        if cfg.cross_mix_stratified:
            for grp in (np.where(in_lower & ok2)[0], np.where((~in_lower) & ok2)[0]):
                if grp.size == 0:
                    continue
                w = float(w_other[grp[0]])                     # constant within group
                k = int(round(w * grp.size))
                if k <= 0:
                    continue
                if k >= grp.size:
                    mask_u[grp] = True
                    continue
                r = rng.random((grp.size, M))
                kth = np.partition(r, k - 1, axis=0)[k - 1]    # per-unit threshold
                mask_u[grp] = r <= kth[None, :]
        else:                                                  # v20 iid Bernoulli
            mask_u = rng.random((n, M)) < w_other[:, None]
            mask_u[~ok2] = False
        return mask_u[:, units]                                # expand to (n, G)

    def _gene_mix(self, anchor, pick, ct_idx, expr_pool, pool_nxy, pool_type,
                  n_genes, rng, pool_m=None):
        r"""v18 -> v23: partial resampling from local real cells of the SAME type.

        v18 drew ONE partner per cell and an iid per-gene mask - novel profiles,
        but every swapped gene decorrelates from its module-mates.  SpatialZ
        goes further (a fresh draw for EVERY gene), maximising novelty and
        maximally destroying covariance.  v23 samples one partner per
        (cell, CO-EXPRESSION MODULE), niche-cosine weighted: whole modules are
        copied intact from partners that share type and micro-environment, so
        the output cloud gains combinatorial novelty (mixing/OT metrics) while
        within-module gene-gene covariance - what Geary's C, co-expression and
        Moran's ranking actually read - survives by construction.

        Returns (partner_pm (n, n_units) int, gene_mask (n, G) bool); the
        caller applies them on whichever scale it emits (log or raw pool)."""
        cfg = self.cfg
        n = anchor.shape[0]
        units = self._mix_units(n_genes)
        M = int(units.max()) + 1
        ...
        if use_niche:
            pm = pool_m / (np.linalg.norm(pool_m, axis=1, keepdims=True) + 1e-9)
        partner_pm = np.repeat(pick[:, None], M, axis=1)
        for i in range(n):
            ci = cand[i]
            ...
            if not ok.size:
                continue
            if use_niche and ok.size > 1:
                w = np.exp(nw * (pm[ok] @ pm[pick[i]]))
                w = w / w.sum()
                partner_pm[i] = rng.choice(ok, size=M, replace=True, p=w)
            else:
                partner_pm[i] = ok[rng.integers(ok.size, size=M)]
        # unit inclusion mask: each (cell, unit) mixes with prob gene_mix_frac,
        # so the EXPECTED fraction of mixed genes matches v18/v20 exactly.
        unit_mask = rng.random((n, M)) < cfg.gene_mix_frac
        unit_mask[np.all(partner_pm == pick[:, None], axis=1)] = False
        gene_mask = unit_mask[:, units]
        # a unit whose partner equals the inherited pick contributes nothing:
        gene_mask &= (partner_pm[:, units] != pick[:, None])
        return partner_pm, gene_mask
    ...
    def _match_composition_local(self, lower, upper, t, ct_idx, expr, anchor,
                                 pool_nxy, pool_e, e_hat, pool_type, pool_expr,
                                 rng, pick, n_lo):
        r"""v23: match the local composition FIELD, not just the histogram.

        The global pass (v14-v20) can be satisfied with every type in the right
        AMOUNT but the wrong PLACE - the cell-type LOCALIZATION metric reads
        exactly that failure.  Here the plane is cut into a comp_bins^2 grid;
        each bin's target composition is the t-interpolation of the two flanks'
        composition WITHIN THAT BIN (Laplace-smoothed; a flank empty in the bin
        defers to the other flank); surplus -> deficit re-grounding then runs
        per bin with the usual local, latent-nearest candidate rule.  Bins with
        fewer than comp_min_cells generated cells are left to the global pass
        that follows (which also mops up any cross-bin residue, so the global
        marginal stays matched)."""
        cfg = self.cfg
        nt = self.n_types
        n = len(ct_idx)
        B = max(int(cfg.comp_bins), 1)
        if B < 2 or n < cfg.comp_min_cells * 2:
            return ct_idx, expr, pick
        lo = anchor.min(0) - 1e-6
        hi = anchor.max(0) + 1e-6
        span = np.maximum(hi - lo, 1e-6)
        def bin_of(xy):
            b = np.clip(((xy - lo) / span * B).astype(int), 0, B - 1)
            return b[:, 0] * B + b[:, 1]
        gbin = bin_of(anchor)
        pbin = bin_of(pool_nxy)
        in_lower_pool = np.arange(pool_nxy.shape[0]) < n_lo
        K = min(cfg.ground_k, pool_nxy.shape[0])
        cand = self._knn(anchor, pool_nxy, K)
        ...
        def comp_of(mask):
            c = np.zeros(nt)
            if mask.any():
                bc = np.bincount(pool_type[mask].astype(int), minlength=nt).astype(float)
                c = (bc + 0.5) / (bc.sum() + 0.5 * nt)          # Laplace smoothing
            return c

        for bid in np.unique(gbin):
            cells = np.where(gbin == bid)[0]
            if cells.size < cfg.comp_min_cells:
                continue
            m_lo = (pbin == bid) & in_lower_pool
            m_hi = (pbin == bid) & (~in_lower_pool)
            if not m_lo.any() and not m_hi.any():
                continue
            c_lo = comp_of(m_lo) if m_lo.any() else comp_of(m_hi)
            c_hi = comp_of(m_hi) if m_hi.any() else comp_of(m_lo)
            target = (1 - t) * c_lo + t * c_hi
            target = target / target.sum()
            nb = cells.size
            tgt = np.floor(target * nb).astype(int)
            rem = nb - tgt.sum()
            if rem > 0:
                for j in np.argsort(-(target * nb - tgt))[:rem]:
                    tgt[j] += 1
            cur = np.bincount(ct_idx[cells], minlength=nt)
            over = [c for c in range(nt) if cur[c] > tgt[c]]
            for uc in range(nt):
                need = tgt[uc] - cur[uc]
                if need <= 0:
                    continue
                pool_cells = [i for i in cells if ct_idx[i] in over]
...
def rank_normalize(X):
    ...
    R = np.empty_like(X)
    n = X.shape[0]
    # v23: AVERAGE ranks for ties.  Sparse count data is mostly zeros; giving
    # the tied zero-mass arbitrary (index-order) ranks injected rank noise that
    # differed between prediction and GT and biased every rank-based metric.
    from scipy.stats import rankdata
    for g in range(X.shape[1]):
        R[:, g] = (rankdata(X[:, g], method="average") - 1.0) / max(n - 1, 1)
    return R
...
def _spearman(a, b):
    """Actual Spearman (rank Pearson).  v20 computed Pearson under a
    'spearman' key; the name and the number now agree."""
    from scipy.stats import rankdata
    ok = ~(np.isnan(a) | np.isnan(b))
    if ok.sum() < 3:
        return np.nan
    return _corr(rankdata(a[ok]).astype(float), rankdata(b[ok]).astype(float))


def _grid_field(xy, values, lo, span, B):
    """Mean of `values` per cell of a B x B grid over [lo, lo+span]."""
    b = np.clip(((xy - lo) / span * B).astype(int), 0, B - 1)
    bid = b[:, 0] * B + b[:, 1]
    sums = np.zeros(B * B); cnts = np.zeros(B * B)
    np.add.at(sums, bid, values); np.add.at(cnts, bid, 1.0)
    out = np.full(B * B, np.nan)
    occ = cnts > 0
    out[occ] = sums[occ] / cnts[occ]
    return out, cnts


def marker_field_r(pred_xy, pR, gt_xy, gR, gene_names, markers, B=10):
    """v23: binned 2-D expression FIELD agreement per marker (the paper's
    marker-field metric, previously benchmark-only).  Both slices are binned on
    the GT's grid; per marker, the per-bin mean rank-expression fields are
    correlated over bins occupied in both."""
    lo = gt_xy.min(0) - 1e-9
    span = np.maximum(gt_xy.max(0) - gt_xy.min(0), 1e-9)
    names = list(gene_names)
    rs = []
    for g in markers:
        if g not in names:
            continue
        col = names.index(g)
        pf, _ = _grid_field(pred_xy, pR[:, col], lo, span, B)
        gf, _ = _grid_field(gt_xy, gR[:, col], lo, span, B)
        rs.append(_corr(pf, gf))
    return float(np.nanmean(rs)) if rs else np.nan


def celltype_localization(pred_xy, pred_types, gt_xy, gt_types, B=10, min_cells=10):
    """v23: per-type spatial density-field agreement (the paper's cell-type
    localization metric, previously benchmark-only).  For each type with at
    least `min_cells` GT cells, the per-bin FRACTION-of-cells fields are
    correlated; the average is weighted by the GT type frequency."""
    if pred_types is None or gt_types is None:
        return np.nan
    lo = gt_xy.min(0) - 1e-9
    span = np.maximum(gt_xy.max(0) - gt_xy.min(0), 1e-9)
    gt_types = np.asarray(gt_types).astype(str)
    pred_types = np.asarray(pred_types).astype(str)
    types, counts = np.unique(gt_types, return_counts=True)
    rs, ws = [], []
    for ty, ct in zip(types, counts):
        if ct < min_cells:
            continue
        pf, _ = _grid_field(pred_xy, (pred_types == ty).astype(float), lo, span, B)
        gf, _ = _grid_field(gt_xy, (gt_types == ty).astype(float), lo, span, B)
        r = _corr(pf, gf)
        if not np.isnan(r):
            rs.append(r); ws.append(float(ct))
    if not rs:
        return np.nan
...
def evaluate_section(pred: VirtualSlice, gt_X, gt_xy, gt_types, gene_names,
    ...
    # (3b) v23: marker FIELD agreement + cell-type LOCALIZATION (the two
    # remaining paper metric families this teaching evaluator was missing).
    out["marker_field_r"] = marker_field_r(pred_xy, pR, gt_xy, gR, gene_names, markers)
    out["celltype_localization"] = celltype_localization(
        pred_xy, pred.cell_type, gt_xy, gt_types)
    ...
    # v23: these are now genuinely Spearman (v20 computed Pearson here).
    pL, gL = normalize_counts(pred_X), normalize_counts(np.asarray(gt_X))
    out["gene_mean_spearman"] = _spearman(pL.mean(0), gL.mean(0))
    out["gene_var_spearman"] = _spearman(pL.var(0), gL.var(0))
...
def main():
    ap = argparse.ArgumentParser(description="Learn SpatialCPA-v23 end to end.")
    ...
    ap.add_argument("--holdout", choices=["alternating", "consecutive"],
                    default="alternating",
                    help="alternating = sections 2,4,6 (narrow gaps, the "
                         "saturated regime); consecutive = sections 3,4,5 "
                         "(one wide gap, the regime v19-v23 target)")
    # ---- benchmark config knobs (defaults = the benchmark flag set) ----------
    ap.add_argument("--edit-weight", type=float, default=0.0)
    ap.add_argument("--ground-blend-flow", type=float, default=1.0)
    ap.add_argument("--ground-k", type=int, default=8)
    ap.add_argument("--ground-temp", type=float, default=0.25)
    ap.add_argument("--ground-keep-margin", type=float, default=1.0)
    ap.add_argument("--type-mode", choices=["inherit", "vote"], default="vote")
    ap.add_argument("--type-vote-k", type=int, default=12)
    ap.add_argument("--gene-mix-frac", type=float, default=0.15)
    ap.add_argument("--gap-scale", type=float, default=3.0)
    ap.add_argument("--alpha-tol", type=float, default=1e-3)
    ap.add_argument("--edit-gap-extra", type=float, default=0.0)
    # ---- v23 knobs (all default ON; turn OFF individually for ablations) -----
    ap.add_argument("--no-module-mix", action="store_true",
                    help="ablation: per-gene chimerism (v20 behaviour)")
    ap.add_argument("--n-gene-modules", type=int, default=8)
    ap.add_argument("--niche-weight", type=float, default=1.0)
    ap.add_argument("--no-stratified", action="store_true",
                    help="ablation: iid Bernoulli cross-mix (v20 behaviour)")
    ap.add_argument("--no-local-composition", action="store_true",
                    help="ablation: global histogram matching only (v20)")
    ap.add_argument("--no-pair-morph", action="store_true",
                    help="ablation: no positional morphing (v20 behaviour)")
    args = ap.parse_args()

    print("=" * 78)
    print("SpatialCPA-v23 (H3D-FLA) — end-to-end learning run over the v3 protocol")
    print("=" * 78)
    ...
    # ---- v3 protocol: 7 sections, flatten z; TWO holdout designs ----
    #   alternating: hold 2,4,6 -> every reconstruction is bracketed by its
    #                immediate neighbours (narrow gaps; the saturated regime).
    #   consecutive: hold 3,4,5 -> a single 4-section-wide gap between 2 and 6
    #                (the wide-gap regime; alpha > 0 and the v19-v23 machinery
    ...
    held_out = (["section_2", "section_4", "section_6"]
                if args.holdout == "alternating"
                else ["section_3", "section_4", "section_5"])
    ...
    # ---- benchmark knobs (the flag set used across all datasets) ----
    ...
    cfg.gene_mix_frac = args.gene_mix_frac
    cfg.gap_scale = args.gap_scale
    cfg.alpha_tol = args.alpha_tol
    cfg.edit_gap_extra = args.edit_gap_extra
    # ---- v23 knobs ----
    cfg.module_mix = not args.no_module_mix
    cfg.n_gene_modules = args.n_gene_modules
    cfg.niche_weight = args.niche_weight
    cfg.cross_mix_stratified = not args.no_stratified
    cfg.composition_local = not args.no_local_composition
    cfg.pair_morph = not args.no_pair_morph
    if args.fast:
    ...
    header = f"{'section':>10}  {'z':>5}  {'morI_r':>7} {'gearC_r':>7} {'mix_pca':>7} " \
             f"{'mrkFld':>7} {'mrkDep':>7} {'ctLoc':>7} {'gmean_r':>7} {'gvar_r':>7} {'n_rat':>6}"
    print(header)
    rows = []
    for sec in held_out:
        z = targets[sec]
        ...
        print(f"{sec:>10}  {z:5.1f}  "
              ...
              f"{scores['embedding_mixing_pca']:7.3f} {scores['marker_field_r']:7.3f} "
              f"{scores['marker_depth_r']:7.3f} {scores['celltype_localization']:7.3f} "
              f"{scores['gene_mean_spearman']:7.3f} {scores['gene_var_spearman']:7.3f} "
              f"{scores['cell_count_ratio']:6.2f}")
    ...
    print(f"{'MEAN':>10}  {'':>5}  "
          ...
          f"{_mean('embedding_mixing_pca'):7.3f} {_mean('marker_field_r'):7.3f} "
          f"{_mean('marker_depth_r'):7.3f} {_mean('celltype_localization'):7.3f} "
          f"{_mean('gene_mean_spearman'):7.3f} {_mean('gene_var_spearman'):7.3f}")

    print("\nHow to read these (all higher = better except that n_rat ~1.0 is the")
    ...
    print("  mrkFld / mrkDep  : marker 2-D FIELD / cortical DEPTH profile agreement.")
    print("  ctLoc            : per-type spatial density-field agreement.")
    print("  gmean_r / gvar_r : per-gene mean / variance Spearman.")
    print("\nv23 ablation map (run each against the default to isolate one idea):")
    print("  --no-module-mix        module vs per-gene chimerism  (mixing vs Geary/co-expr)")
    print("  --no-stratified        stratified vs iid cross-mix   (field noise at wide gaps)")
    print("  --no-local-composition local vs global composition   (cell-type localization)")
    print("  --no-pair-morph        morph vs static positions     (fields at wide gaps)")
    print("  --holdout consecutive  the wide-gap regime: ALL v23 features act")
    print("                         (cross-mix, stratification and pair-morph are")
    print("                         alpha-gated and only fire here).")
    print("  --holdout alternating  the narrow-gap regime: alpha snaps to 0, so only")
    print("                         module chimerism, niche weighting and local")
    print("                         composition act - the levers for mixing and")
    print("                         cell-type localization where spatial metrics")
    print("                         are already saturated.")
