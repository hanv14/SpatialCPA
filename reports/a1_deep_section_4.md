...
| panel | A1n. permutation null (real counts shuffled) | **+0.0382** | -0.2487 .. +0.3132 (20) | +0.1092 | 0.3313 | 32 |
...
| all genes | A1n. permutation null (real counts shuffled) | **-0.0035** | -0.1463 .. +0.0598 (20) | +0.0794 | 0.0472 | 1017 |
...
|---|---|---|---|
| **R1** `corr(I_real, control)` — is the tissue's ordering a sparsity ordering? | +0.6787 | +0.7163 | +0.7690 |
| **R2** `corr(I_4, control)` | +0.9027 | +0.7663 | +0.7720 |
| **R3** partial `corr(I_4, I_real given control)` | +0.2797 | +0.0927 | -0.0087 |
| retained fraction of `r` = +0.7306 | 38.3% | 12.7% | -1.2% |

**UNINFORMATIVE — the control specifications disagree** — the 3 specifications span 0.395 against a 0.150 tolerance (`ladder_preregistration.md` §4, `flanking_copy_preregistration.md` §3).
...
`spatial_scramble` keeps every per-gene marginal and destroys only position: **-0.0035** (sd 0.0607, 20 seeds).
...
|---|---|---|---|---|
| 10 genes | **+0.4989** | +0.4586 .. +0.5367 | 50.5% | 20 |
| 25 genes | **+0.5049** | +0.4601 .. +0.5429 | 51.1% | 20 |
| 50 genes | **+0.5139** | +0.4716 .. +0.5559 | 52.0% | 20 |
...
| retained fraction of `r` = +0.9886 | 99.0% | 98.6% | 97.9% |
...
`F3_copy` = **+0.5049** (median over widths [10, 25, 50], span 0.015); `r_copy` = **+0.9886**; denominator **+0.4836**.

| rung | raw `r` | its own F3 (abundance alone) | **rescaled** |
|---|---|---|---|
| flanking_copy | +0.9886 | +0.5049 | **1.00** |
| A1c | +0.9699 | +0.5654 | **0.96** |
| A1b-t | +0.9522 | +0.5888 | **0.92** |
| A1b-p | +0.8681 | +0.6564 | **0.75** |
| A1b | +0.8242 | +0.6459 | **0.66** |
| 4 | +0.7306 | +0.6530 | **0.47** |
| A1a | +0.6732 | +0.5770 | **0.35** |
| A1n | +0.0594 | +0.0697 | **-0.92** |
