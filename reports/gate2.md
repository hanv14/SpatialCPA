...
2026-08-25 05:38 UTC, Python 3.11.15 on
Linux-6.18.44-fc-v21-x86_64-with-glibc2.39 (x86_64, 4 cpus, 4 torch threads, torch 2.2.2+cu121).
...
| retrieval neighbourhoods | 32.2 of 64, at a 3 deg probe |
...
| **G2.1h-c** | MUTATION: querying **retrieval** in the model frame instead of the data frame changes this many of the K neighbours per cell, on average, at a **fixed 3 degree** probe rotation<br><sub>symmetric difference, so the ceiling is 2K = 64; this sits at 50% of it. Measured at a small fixed rotation about (1.0, 1.0, 1.0) **on purpose**: at the gate's own rotation the quantity saturates and stops being a measurement (SPEC_QUESTIONS C32). Read a fall here as retrieval becoming less sensitive to its query frame</sub> | 32.1641 | `> 0` | **PASS** |
| **G2.1h-c2** | the same mutation at the gate's **own** augmentation rotation. Retained as a binary wired/not-wired check only<br><sub>out of a ceiling of 2K = 64. At 179.7 degrees the two neighbour sets are disjoint, so this pins near its ceiling for any real rotation and its magnitude means nothing — C32. G2.1h-c is the graded form</sub> | 63.9697 | `> 0` | **PASS** |
