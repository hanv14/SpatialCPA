...
**Verdict: FAIL** — failing criteria: G1.3c, G1.3d, G1.4b

Generated by `python scripts/gate1_report.py --seed 0` on
2026-08-15 02:37 UTC, Python 3.12.14 on
macOS-26.5.2-arm64-arm-64bit. Fixture: `make_synthetic_volume(seed=0)` —
...
| **G1.3a** | median \|I_gen - I_real\| with the GRF prior, as a fraction of the same quantity with an i.i.d. prior<br><sub>GRF 0.0776 vs i.i.d. 0.3956</sub> | 0.19624 | `< 0.5` | **PASS** |
| **G1.3b** | Pearson r between per-gene I_gen and I_real, GRF prior | 0.819849 | `> 0.7` | **PASS** |
...
| **G1.3e** | Pearson r with the i.i.d. prior (the spec asks only that it be lower)<br><sub>GRF r = 0.8198</sub> | 0.347565 | - | **REPORT** |
...
| **G1.4b** | wall clock to query 1,000,000 points at M = 4096, d_h = 64<br><sub>single CPU, no GPU; hardware-dependent (SPEC_QUESTIONS B9)</sub> | 6.06555 s | `< 5` | **FAIL** |
...
| **D1a** | smallest step of median I_gen over the same 0.25x - 4x sweep<br><sub>0.25x: 0.2120, 0.5x: 0.3633, 1x: 0.4704, 2x: 0.4875, 4x: 0.4825</sub> | -0.00504306 | - | **REPORT** |
...
million queries at `M = 4096`, `d_h = 64` take 6.07 s on this 4-core CPU.
...
`ell` (0.25x: 0.2714, 0.5x: 0.3895, 1x: 0.4179, 2x: 0.4128, 4x: 0.3478). It rises to a peak near `1.6x` the fitted length-scale and then falls; the
...
(0.25x: 0.2120, 0.5x: 0.3633, 1x: 0.4704, 2x: 0.4875, 4x: 0.4825), which locates the cause in the ratio of `ell` to the field of view rather
than in the field.
