...
2026-08-15 03:22 UTC, Python 3.12.14 on
...
| **G1.3a** | median \|I_gen - I_real\| with the GRF prior, as a fraction of the same quantity with an i.i.d. prior<br><sub>GRF 0.0554 vs i.i.d. 0.4233</sub> | 0.130895 | `< 0.5` | **PASS** |
| **G1.3b** | Pearson r between per-gene I_gen and I_real, GRF prior | 0.91669 | `> 0.7` | **PASS** |
| **G1.3c** | median I_gen is monotone increasing over the sweep points inside the calibration bracket, ell <= min(0.2 x the 3000 um in-plane extent, 2 x the fitted ell) (smallest step shown)<br><sub>sweep points kept: 0.25x = 26 um / 0.5x = 51 um / 1x = 103 um / 2x = 206 um; cap = 206 um</sub> | 0.0284954 | `> 0` | **PASS** |
| **G1.3d** | \|fitted ell - best-matching ell\| / best-matching ell<br><sub>fitted ell_xy = 102.9 um, best match at 1.09x = 111.9 um</sub> | 0.0807721 | `< 0.25` | **PASS** |
| **G1.3g-a** | I_gen(ell) is unimodal: largest rise after the maximum or fall before it, against twice the curve's own standard error<br><sub>peak at 2.51984x, mean SE 0.0035</sub> | 0 | `< 0.0069188` | **PASS** |
...
| **G1.3e** | Pearson r with the i.i.d. prior (the spec asks only that it be lower)<br><sub>GRF r = 0.9167</sub> | 0.377089 | - | **REPORT** |
| **G1.3f** | median \|C_gen - C_real\|, GRF as a fraction of i.i.d. (Geary's C)<br><sub>GRF 0.0550 vs i.i.d. 0.4236</sub> | 0.129861 | - | **REPORT** |
| **G1.4a** | values differing between two processes at the same seed (4096 x 64) | 0 | `== 0` | **PASS** |
| **G1.4b** | throughput at M = 4096, d_h = 64, on this machine (arm, 12 cpus, 12 torch threads, torch 2.2.2)<br><sub>1,000,000 points in 5.92 s. Reference: 290,000 points/s on 4-core Intel Xeon @ 2.10 GHz (AVX-512), 4 torch threads, torch 2.2.2 CPU, no GPU. Recorded rather than asserted: a wall-clock threshold makes the gate a statement about the machine that ran it, and the same code measured 3.4 s on the reference box and 6.1 s on an Apple-silicon laptop</sub> | 168961 points/s | - | **REPORT** |
| **G1.4c** | cost scales linearly: querying 1,000,000 points against 125,000 (8x the points; ideal ratio 8)<br><sub>0.910 s -> 5.919 s. Dimensionless, so unlike a wall clock it means the same thing on every machine</sub> | 6.50118x | `< 12` | **PASS** |
...
| **D1a** | smallest step of median I_gen over the full 0.25x - 4x sweep<br><sub>0.25x: 0.2955, 0.5x: 0.4120, 1x: 0.4439, 2x: 0.4128, 4x: 0.3298</sub> | -0.0830177 | - | **REPORT** |
...
168,961 points/s here against 290,000 points/s on the reference
machine, and 8x the points cost 6.5x the time — linear, where anything quadratic
would be 64x.
...
prior gives (0.0554 vs 0.4233) — the spec asks for less than 50% — and the
...
the window and the decline is far steeper (0.25x: 0.2955, 0.5x: 0.4120, 1x: 0.4439, 2x: 0.4128, 4x: 0.3298) — the finding that raised
...
with a smallest step of +0.0285. Second, the extent cap of 0.2 is about
