...
2026-08-24 21:44 UTC, Python 3.11.15 on
...
| **G1.4b** | throughput at M = 4096, d_h = 64, on this machine (x86_64, 4 cpus, 4 torch threads, torch 2.2.2+cu121)<br><sub>1,000,000 points in 2.77 s. Reference: 290,000 points/s on 4-core Intel Xeon @ 2.10 GHz (AVX-512), 4 torch threads, torch 2.2.2 CPU, no GPU. Recorded rather than asserted: a wall-clock threshold makes the gate a statement about the machine that ran it, and the same code measured 3.4 s on the reference box and 6.1 s on an Apple-silicon laptop</sub> | 360756 points/s | - | **REPORT** |
| **G1.4c** | cost scales linearly: querying 1,000,000 points against 125,000 (8x the points; ideal ratio 8)<br><sub>0.353 s -> 2.772 s. Dimensionless, so unlike a wall clock it means the same thing on every machine</sub> | 7.85601x | `< 12` | **PASS** |
...
360,756 points/s here against 290,000 points/s on the reference
machine, and 8x the points cost 7.9x the time — linear, where anything quadratic
would be 64x.
