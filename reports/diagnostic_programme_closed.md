...
all of them a perfect result. And `A1a` — the encoder's own posterior on the truth, decoded and
emitted — sits at **+0.6732**, *below* stage 4's **+0.7306**, with the positional advantage on the
wrong side. `ladder_preregistration.md` §2a makes a low rung with that advantage conclusive: a
...
- `paper_morans_pearson`'s copy floor is real spatial fidelity, and we lose to it.
...
`specs/10` §4.2's rules — §4.2f-i, §4.2k, §4.2l, §4.2m and now §4.2n — were all earned by this
