"""
Lightweight data containers for SpatialCPA-v14 (self-contained).

A :class:`Slice` is one aligned tissue section; a :class:`SliceStack` is an ordered
stack (by z) of sections. All coordinates are physical ``(x, y, z)``. In the benchmark
the stack only ever contains the *training-only, re-registered* sections — the held-out
slice never enters it (enforced upstream by ``benchmark-pbya-v2``), which realizes the
pipeline's leave-one-slice-out / gap-aware protocol.
"""
