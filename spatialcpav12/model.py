"""
SpatialCPA-v12 — generative continuous 3D virtual-slice model (orchestration).

Ties together the neighbouring-slice context encoder, the Stage-1 layout field and
the Stage-2 *generative* expression decoder, the foundation-model teacher, and all
losses, into a model that trains per stack (leave-one-slice-out self-supervision +
distillation) and is queried at arbitrary continuous ``z`` (``generate_virtual_slice``).
Positions and types are produced first (Stage 1); expression is *sampled* from the
factor-analysis decoder conditioned on the layout (Stage 2). If PyTorch is unavailable
or training fails, a deterministic nearest-slice fallback keeps the method usable.
"""
...
from .config import SpatialCPAv12Config
...
class SpatialCPAv12:
    def __init__(self, stack: SliceStack, gene_names: Sequence[str],
                 ...
                 cfg: Optional[SpatialCPAv12Config] = None,
        ...
        self.cfg = cfg or SpatialCPAv12Config()
    ...
    def _fit(self):
        try:
            ...
            print(f"[spatialcpav12] PyTorch unavailable ({e}); nearest-slice fallback.")
            return
        ...
        # nearest-slice fallback). Only give up to the nearest-slice fallback if CPU fails.
        ...
        for attempt in ("device", "cpu"):
            try:
                ...
                if attempt == "device" and oom and not self._force_cpu:
                    print(f"[spatialcpav12] CUDA out of memory; retrying training on CPU. "
                          f"(free a GPU or pass --device cpu to skip this.)")
                    self._force_cpu = True
                    continue
                import traceback
                print(f"[spatialcpav12] training failed ({e}); nearest-slice fallback.")
    ...
    def generate_virtual_slice(self, z: float) -> VirtualSlice:
        if self.trained:
            try:
                ...
                print(f"[spatialcpav12] inference failed ({e}); nearest-slice fallback.")
