...
def resolve_symbols(gene_names, gene_symbols=None, symbol_map_path=None):
    ...
    if m:
        ...
        print(f"[spatialcpav12] symbol map applied: {n_mapped}/{len(names)} genes translated.")
        return syms
    if _looks_ensembl(names):
        print("[spatialcpav12] WARNING: panel looks like Ensembl IDs but no symbol map "
...
def build_teacher(cfg, stack, gene_names, gene_symbols=None):
    ...
    if want is not None and want in TEACHER_REGISTRY:
        try:
            ...
            print(f"[spatialcpav12] teacher: real {want} "
                  f"(weights={cfg.weights_path or cfg.gene_embedding_path}).")
            return t
        except Exception as e:
            if cfg.kind != "auto":
                print(f"[spatialcpav12] real teacher '{want}' failed ({e}).")
                if cfg.kind == want:  # explicitly requested -> do not silently downgrade
                    raise
            else:
                print(f"[spatialcpav12] real teacher '{want}' unavailable ({e}); using proxy.")
    print("[spatialcpav12] teacher: data-derived proxy stand-in "
