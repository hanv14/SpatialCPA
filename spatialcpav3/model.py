"""
SpatialCPA v3 — main model class.

Assembles the Fourier encoder, spatial backbone, and prediction heads into a
single end-to-end differentiable model. Adds a generative expression mode
(``expression_mode='gaussian'``) and a :meth:`sample_expression` method so the
v3 virtual-slice generator can draw stochastic profiles.
"""
...
from spatialcpav3.heads import (
    ClassifierHead,
...
class SpatialCPA(nn.Module):
    ...
    def forward(self, coords, cell_type_idx=None):
        ...
        Returns dict with 'h', 'cell_type_logits', 'region_logits', and the
        expression outputs for the active mode ('predicted_expr' for mse,
        'expr_mu'/'expr_logvar' for gaussian, 'mu'/'theta'/'pi_logits' for zinb).
        """
